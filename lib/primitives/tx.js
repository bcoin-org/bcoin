...
const Taproot = require('../script/taproot');
...
class TX {
  ...
  constructor(options) {
    ...
    // BIP143 Witness V0 cached hashes (double SHA256)
    this._hashPrevouts = null;
    this._hashSequence = null;
    this._hashOutputs = null;

    // BIP341 Witness V1 cached hashes (single SHA256)
    this._singlehashPrevouts = null;
    this._singlehashSequence = null;
    this._singlehashOutputs = null;
    this._singlehashAmounts = null;
    this._singlehashScripts = null;
   ...
   * p2pkh script). Not used for Tapscript
  ...
  signatureHash(index, prev, value, type, digest, coins, pos) {
    ...
    assert(!prev || prev instanceof Script);
  ...
  signatureHashWitnessV0(index, prev, value, type) {
    ...
    if (!(type & hashType.ANYONECANPAY)) {
      ...
      } else if (this._singlehashPrevouts) {
        prevouts = sha256.digest(this._singlehashPrevouts);
        if (!this.mutable)
    ...
    if (!(type & hashType.ANYONECANPAY)
      ...
      } else if (this._singlehashSequence) {
        sequences = sha256.digest(this._singlehashSequence);
        if (!this.mutable)
    ...
    if ((type & 0x1f) !== hashType.SINGLE
      ...
      } else if (this._singlehashOutputs) {
        outputs = sha256.digest(this._singlehashOutputs);
        if (!this.mutable)
  ...
  signatureHashTaproot(index, value, type, coins, pos) {
    ...
    if (!(type & hashType.ANYONECANPAY)) {
      if (this._singlehashPrevouts) {
        prevouts = this._singlehashPrevouts;
      } else {
        const bw = bio.pool(this.inputs.length * 36);
        ...
        if (!this.mutable)
          this._singlehashPrevouts = prevouts;
      }

      if (this._singlehashAmounts) {
        amounts = this._singlehashAmounts;
      } else {
        ...
        if (!this.mutable)
          this._singlehashAmounts = amounts;
      }

      if (this._singlehashScripts) {
        scripts = this._singlehashScripts;
      } else {
        let size = 0;
        for (const coin of coins)
          size += coin.script.getVarSize();

        const bw = bio.pool(size);

        for (const coin of coins)
          coin.script.toWriter(bw);

        scripts = sha256.digest(bw.render());

        if (!this.mutable)
          this._singlehashScripts = scripts;
      }

      if (this._singlehashSequence) {
        sequences = this._singlehashSequence;
      } else {
        const bw = bio.pool(this.inputs.length * 4);
        ...
        if (!this.mutable)
          this._singlehashSequence = sequences;
      }

      // Four hash comitments above
      txDataSize += 32 * 4;

      // Also commit to this input's index in the TX
      txDataSize += 4;
    } else {
      // ANYONECANPAY commits to only this input's
      // outpoint / amount / scriptPubkey / sequence
      txDataSize += 36 + 8 + 35 + 4;
    }

    if ((type & 0x1f) !== hashType.SINGLE
        && (type & 0x1f) !== hashType.NONE) {
      if (this._singlehashOutputs) {
        outputs = this._singlehashOutputs;
      } else {
        let size = 0;
        ...
        if (!this.mutable)
          this._singlehashOutputs = outputs;
      }
    ...
    if (spendType & (1 << 1)) {
      ...
      const script = Script.fromRaw(input.witness.getTapleaf());
      const tapleaf = new Taproot.TapLeaf(script, leafVersion);
      bw.writeHash(tapleaf.getLeafHash());
  ...
  hasStandardWitness(view) {
    ...
    for (const input of this.inputs) {
      ...
      // Bitcoin Core
      // src/policy/policy.cpp IsWitnessStandard()
      if (prev.isPayToTaproot() && !coin.script.isScripthash()) {
        ...
        // Key path spend (1 item besides annex)
        // (no policy rules apply)
        if (witness.items.length === 1)
          continue;

        // 2 or more items (besides annex) in witness means script path spend
        // Empty control block is invalid
        const controlBlock = witness.getControlBlock();
        if (controlBlock && controlBlock.length === 0)
          return false;

        // Leaf version Tapscript
        if (
            controlBlock &&
            ((controlBlock[0] & 0xfe) === common.LEAF_VERSION_TAPSCRIPT)
          ) {
          // Check size of remaining stack items, ignoring:
          // - annex (already returned false if present)
          // - script (penultimate item)
          // - control block (last item)
          for (const item of witness.items.slice(0, -2)) {
            if (item.length > policy.MAX_P2WSH_PUSH)
              return false;
          }
        }
