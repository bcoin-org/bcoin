...
TX.prototype.hasStandardWitness = function hasStandardWitness(view) {
  var i, input, witness, coin, prev, redeem, m;
  ...
  for (input of this.inputs) {
    ...
    if (prev.isWitnessScripthash()) {
      ...
      for (i = 0; i < witness.items.length - 1; i++) {
      ...
      if (prev.isMultisig()) {
        ...
        for (i = 1; i < witness.items.length - 1; i++) {
...
TX.prototype.checkContext = function checkContext(view, height) {
  var total = 0;
  var input, coins, coin, fee, value;
