...
class MTX extends TX {
   ...
   * @param {Number} index - Index of input being signed.
   * @param {KeyRing[]} rings - with private key.
  ...
  signCustomInput(index, rings, redeemScript, witnessScript, type, coin) {
    function getValue(tx, coin, input) {
      if (coin) {
        if (Coin.isCoin(coin) || Output.isOutput(coin))
          return coin.value;
      ...
      const output = tx.view.getOutput(prevout);
      if (!output)
        throw new Error('Cannot get value of spending output');
