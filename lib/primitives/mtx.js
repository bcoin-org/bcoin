...
class CoinSelector {
  ...
  fromOptions(options) {
    ...
    if (options.useSelectByValue !== undefined) {
      assert(typeof options.useSelectByValue === 'boolean');
      this.useSelectByValue = options.useSelectByValue;
    }
  ...
  async select(coins) {
    ...
    if (this.hardFee !== -1) {
      this.selectHard();
    } else if (this.useSelectByValue && this.selection === 'value') {
  ...
  /**
   * Initiate selection using new selection algorithm.
   */
  ...
  async selectByValue() {
    ...
    // calculate fee for everything except inputs.
    let size = await this.tx.estimateSize();
    const fee = this.getFee(size);
    // add fee to target value.
    target += fee;
    ...
    let effectiveValue = await this.getEffectiveValue(coin);
    while (target >= effectiveValue) {
      ...
      effectiveValue = await this.getEffectiveValue(coin);
    }

    if (target > 0 && this.index !== this.coins.length) {
      ...
      const closestCoin = await this.findSmallestGreater(target);
      this.tx.addCoin(closestCoin);
      this.chosen.push(closestCoin);
    }
  ...
  async findSmallestGreater(target) {
    let i = this.index;
    let j = this.coins.length - 1;
    // Corner cases
    // if target < smallest coin available
    // return the smallest coin
    if (target <= await this.getEffectiveValue(this.coins[j]))
      return this.coins[j];
    ...
    while (i < j) {
      ...
      const coin = this.coins[mid];
      const effectiveValue = await this.getEffectiveValue(coin);
      ...
      if (target < effectiveValue) {
        if (mid < this.coins.length - 1) {
          // calculate effective value of coin at mid + 1
          const next = this.coins[mid + 1];
          const nextEffectiveValue = await this.getEffectiveValue(next);
