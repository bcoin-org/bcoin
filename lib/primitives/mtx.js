...
class MTX extends TX {
  ...
  /**
   * Sign custom P2WSH input.
   * @param {Number} index - Index of input being signed.
   * @param {KeyRing|KeyRing[]} rings - with private key.
   * Sorted by desired signature order.
   * @param {Script} redeemScript - its hash is in spending output.
   * @param {Buffer|Number[]} witnessScript - signatures template.
   * witnessScript is the SegWit equivalent of scriptSig.
   * Put an empty buffer as placeholder for each signature.
   * @param {SighashType} type
   * @param {Coin|Output|Number} coin or output or value.
   * @returns {Boolean} Whether the input was able to be signed.
   */

  signCustomInput(
  index, rings, redeemScript, witnessScript, type, coin, flags) {
    function validate(
    tx, index, rings, redeemScript, witnessScript, type, coin, flags) {
      assert(index >>> 0 === index, 'index must be integer');

      for (const ring of rings)
        assert(KeyRing.isKeyRing(ring),
        'rings should be array of instances of KeyRing');

      assert(Script.isScript(redeemScript),
      'redeemScript must be an instance of Script');
      assert(redeemScript.isUnknown(), 'redeemScript should be custom');

      assert(Array.isArray(witnessScript), 'witnessScript must be an array');
      for (const op of witnessScript)
        assert(Buffer.isBuffer(op) || (op >>> 0 === op),
        'witnessScript elements must be buffers or integers');

      assert((!type) || (type >>> 0 === type),
      'type must be null, undefined or a valid input type');

      assert(!(coin) || Coin.isCoin(coin)
      || Output.isOutput(coin) || (coin >>> 0 === coin),
      'coin must be undefined, null, integer, Output or Coin');

      let script;
      if (!coin)
        script = tx.view.getOutputFor(tx.inputs[index]).script;
      else if (Coin.isCoin(coin) || Output.isOutput(coin))
        script = coin.script;
      // we don't know spending output if user passes value, oh well
      if (!(coin >>> 0 === coin))
        assert(script.isWitnessScripthash(), 'input must spend P2WSH');

      if (flags)
        assert(flags >>> 0 === flags, 'flags must be valid verify flags');
    }

    function getCoin(tx, coin, input, redeemScript) {
      if (Coin.isCoin(coin))
        return coin;

      if (!coin)
        return tx.view.getOutputFor(input);

      let value;
      if (coin >>> 0 === coin)
        value = coin;
      else if (Output.isOutput(coin))
        value = coin.value;

      return new Coin({
        value,
        script: Script.fromProgram(0, redeemScript.sha256())
      });
    }
    ...
    validate(this, index, rings, redeemScript, witnessScript, type, coin);

    const input = this.inputs[index];
    coin = getCoin(this, coin, input, redeemScript);
    assert(coin, 'Cannot get value of spending output');

    const sigs = [];
    for (const ring of rings) {
      sigs.push(this.signature(index, redeemScript, coin.value,
        ring.privateKey, type, version));
    }

    const stack = Stack.fromTemplate(witnessScript, sigs);
