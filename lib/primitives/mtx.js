...
class MTX extends TX {
    ...
    if (sh) {
      ...
      if (redeem.isProgram()) {
        ...
        if (wsh) {
          ...
          witness.push(wredeem.encode());

          input.witness.fromStack(witness);
          input.script.fromItems([redeem.encode()]);
  ...
  getJSON(network, entry, index) {
    return super.getJSON(network, this.view, entry, index);
  }
