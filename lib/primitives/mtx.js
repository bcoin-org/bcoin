...
class MTX extends TX {
  ...
  signCustomInput(index, rings, redeemScript, witnessScript, type, coin) {
    function validate(
    tx, index, rings, redeemScript, witnessScript, type, coin) {
    ...
    function getValue(tx, coin, input) {
      if (!coin)
        coin = tx.view.getOutputFor(input);
    ...
    const input = this.inputs[index];
    const value = getValue(this, coin, input);

    const sigs = [];
    for (const ring of rings) {
      sigs.push(this.signature(index, redeemScript, value,
