...
const {ChainFlags, ChainState} = require('./internal/records');
const {StateCache} = require('./internal/cache');
...
class ChainDB {
  ...
  async connectBlock(entry, block, view) {
    ...
    this.connectChainState(entry, block, view);
  ...
  connectChainState(entry, block, view) {
    for (let i = 0; i < block.txs.length; i++) {
      const tx = block.txs[i];
      ...
      if (i > 0) {
        ...
        // coinbase txids and should not increment
        // the statistics.
  ...
  async disconnectBlock(entry, block) {
    ...
    for (let i = block.txs.length - 1; i >= 0; i--) {
      ...
      if (i > 0) {
        ...
        // Blocks 91842 and 91880 created duplicate
        // coinbase txids and should not remove coins
        // or decrement the statistics.
        const hash = this.network.bip30[entry.height];
        if (hash && hash.equals(entry.hash))
          continue;
      }
