...
CoinCache.prototype.start = function start() {
  assert(!this.written);
};

CoinCache.prototype.drop = function drop() {
  this.written = false;
};

CoinCache.prototype.commit = function commit() {
  if (!this.written)
    return;
...
CoinCache.prototype.get = function get(hash, index) {
  const outputs = this.map.get(hash);
...
CoinCache.prototype.set = function set(hash, index, coin) {
  if (!this.map.has(hash)) {
    this.size += 80;
    this.map.set(hash, new Map());
  }
  ...
  outputs.set(index, raw);
};

CoinCache.prototype.needsFlush = function needsFlush() {
  if (this.size > this.capacity)
    return true;

  return this.last - util.now() > 10;
};

CoinCache.prototype.flush = function flush(batch, state, tip) {
  assert(!this.written);

  for (const [hash, outputs] of this.map) {
    for (const [index, coin] of outputs) {
      if (!coin) {
        batch.del(layout.c(hash, index));
        continue;
      }

      batch.put(layout.c(hash, index), coin);
    }
  }

  const bw = new StaticWriter(56);
  bw.writeHash(tip);
  bw.writeU64(state.tx);
  bw.writeU64(state.coin);
  bw.writeU64(state.value);

  const raw = bw.render();

  batch.put(layout.K, raw);
...
ChainDB.prototype.syncCoinCache = async function syncCoinCache() {
  const state = await this.getCoinState();

  if (!state) {
    await this.db.put(layout.K, this.state.toRaw());
    return;
  }

  if (state.tip === this.state.tip)
    return;

  let tip = await this.getTip();
  assert(tip);

  this.logger.debug('Resetting main chain to: %s.', state.rhash());

  const batch = this.db.batch();

  batch.put(layout.p(state.tip), null);
  batch.del(layout.p(tip.hash));
  batch.put(layout.R, state.toRaw());

  while (tip.hash !== state.tip) {
  ...
  this.cacheHeight.reset();
  this.cacheHash.reset();

  this.state = state;
};

ChainDB.prototype.startTimer = function startTimer() {
  assert(this.timer == null);
  this.timer = co.setInterval(this.flushCoins, 20000, this);
};

ChainDB.prototype.stopTimer = function stopTimer() {
  assert(this.timer != null);
  co.clearInterval(this.timer);
  this.timer = null;
};

ChainDB.prototype.flushCoins = async function flushCoins() {
  const batch = this.db.batch();
  this.coinCache.flush(batch, this.state, this.state.tip);
  await batch.write();
};
...
/**
 * Retrieve the utxo hash record.
 * @returns {Promise}
 */

ChainDB.prototype.getCoinState = async function getCoinState() {
  const data = await this.db.get(layout.K);
...
ChainDB.prototype.saveCoinState = async function saveCoinState(state) {
  await this.db.put(layout.K, state.toRaw());
};
...
ChainDB.prototype.readCoin = async function readCoin(prevout) {
  ...
  const cache = this.coinCache.get(hash, index);
...
ChainDB.prototype.saveView = function saveView(view, tip, force) {
  for (const [hash, coins] of view.map) {
    for (const [index, coin] of coins.outputs)
      this.coinCache.set(hash, index, coin);
  }

  if (force || this.coinCache.needsFlush()) {
    this.logger.debug(
      'Flushing %d items in cache (%dmb).',
      this.coinCache.items,
      util.mb(this.coinCache.size));

    this.coinCache.flush(this.current, this.pending, tip);
  }
};
...
ChainDB.prototype.connectBlock = async function connectBlock(entry, block, view) {
  ...
  this.saveView(view, entry.hash, false);
...
ChainDB.prototype.disconnectBlock = async function disconnectBlock(entry, block) {
  ...
  // Commit new coin state.
  this.saveView(view, entry.prevBlock, true);
