...
const CacheDB = require('../db/cachedb');
...
function ChainDB(options) {
  ...
  this.db = new CacheDB(this.db);
  this.db.debug = (...args) => this.logger.warning(...args);
  this.store = createStore(this.db, this.options);
...
ChainDB.prototype.syncStore = async function syncStore() {
  if (this.options.spv)
    return;
  ...
  for (;;) {
    ...
    try {
      raw = await this.store.readBlock(entry.hash);
    } catch (e) {
      ;
    }
...
ChainDB.prototype.prune = async function prune() {
  ...
  for (let i = start; i <= end; i++) {
    ...
    await this.store.pruneBlock(hash, batch);
  }
...
ChainDB.prototype.getUndoCoins = async function getUndoCoins(hash) {
  const raw = await this.store.readUndo(hash);

  if (!raw)
    return new UndoCoins();

  return UndoCoins.fromRaw(raw);
};
...
ChainDB.prototype.getRawBlock = async function getRawBlock(block) {
  ...
  return this.store.readBlock(hash);
};
...
ChainDB.prototype._removeChain = async function _removeChain(hash) {
  ...
  for (;;) {
    ...
    await this.store.removeBlock(hash, this);
...
ChainDB.prototype.saveBlock = async function saveBlock(entry, block, view) {
  ...
  await this.store.writeBlock(hash, block.toRaw(), this);
...
ChainDB.prototype.removeBlock = async function removeBlock(entry) {
  ...
  await this.store.removeBlock(entry.hash, this);
...
ChainDB.prototype.saveView = function saveView(view) {
  for (const [hash, coins] of view.map) {
    for (const [index, coin] of coins.outputs) {
      ...
      this.put(layout.c(hash, index), coin.toRaw());
    }
  }
};
...
ChainDB.prototype.connectBlock = async function connectBlock(entry, block, view) {
  ...
  if (!view.undo.isEmpty()) {
    const raw = view.undo.commit();
    await this.store.writeUndo(hash, raw, this);
  }
...
ChainDB.prototype.pruneBlock = async function pruneBlock(entry) {
  ...
  await this.store.pruneBlock(hash, this);
};
...
ChainDB.prototype.saveFlags = function saveFlags() {
  ...
  this.db.put(layout.O, flags.toRaw());
