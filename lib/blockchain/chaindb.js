...
function ChainDB(options) {
  ...
  this.b = {
    state: this.db.bucket('R'),
    flags: this.db.bucket('O'),
    deploys: this.db.bucket('v'),
    entry: this.db.bucket('e', ['hash256']),
    height: this.db.bucket('h', ['hash256']),
    hash: this.db.bucket('H', ['uint32']),
    next: this.db.bucket('n', ['hash256']),
    tips: this.db.bucket('p', ['hash256']),
    block: this.db.bucket('b', ['hash256']),
    tx: this.db.bucket('t', ['hash256']),
    coin: this.db.bucket('c', ['hash256', 'uint32']),
    undo: this.db.bucket('u', ['hash256']),
    bit: this.db.bucket('v', ['uint8', 'hash256']),
    itx: this.db.bucket('T', ['phash', 'hash256']),
    icoin: this.db.bucket('C', ['phash', 'hash256', 'uint32'])
  };
...
ChainDB.prototype.getHeight = async function getHeight(hash) {
  ...
  const height = await this.b.height.get(hash);
...
ChainDB.prototype.getHash = async function getHash(height) {
  ...
  const hash = await this.b.hash.get(height);
...
ChainDB.prototype.getEntryByHeight = async function getEntryByHeight(height) {
  ...
  const data = await this.b.hash.get(height);
...
ChainDB.prototype.getEntryByHash = async function getEntryByHash(hash) {
  ...
  const raw = await this.b.entry.get(hash);
...
ChainDB.prototype.getState = async function getState() {
  const data = await this.b.state.get();
...
ChainDB.prototype.getFlags = async function getFlags() {
  const data = await this.b.flags.get();
...
ChainDB.prototype.getStateCache = async function getStateCache() {
  ...
  const items = await this.b.bit.range();

  for (const item of items) {
    const [bit, hash] = item.key;
...
ChainDB.prototype.writeDeployments = function writeDeployments() {
...
ChainDB.prototype.checkDeployments = async function checkDeployments() {
  const raw = await this.b.deploys.get();
...
ChainDB.prototype.verifyDeployments = async function verifyDeployments() {
  ...
  for (const bit of invalid) {
    ...
    const keys = await this.b.bit.keys();

    for (const [bit, hash] of keys)
      this.b.bit.del(bit, hash);
  }
...
ChainDB.prototype.prune = async function prune() {
  ...
  for (let i = start; i <= end; i++) {
    ...
    this.b.block.del(hash);
    this.b.undo.del(hash);
  }
...
ChainDB.prototype.getNextHash = async function getNextHash(hash) {
  const data = await this.b.next.get(hash);
...
ChainDB.prototype.readCoin = async function readCoin(prevout) {
  ...
  const raw = await this.b.coin.get(hash, index);
...
ChainDB.prototype.hasCoins = async function hasCoins(tx) {
  for (let i = 0; i < tx.outputs.length; i++) {
    if (await this.b.coin.has(tx.hash(), i))
      return true;
  }
  return false;
};
...
ChainDB.prototype.getUndoCoins = async function getUndoCoins(hash) {
  const data = await this.b.undo.get(hash);
...
ChainDB.prototype.getRawBlock = async function getRawBlock(block) {
  ...
  return await this.b.block.get(hash);
};
...
ChainDB.prototype.getMeta = async function getMeta(hash) {
  ...
  const data = await this.b.tx.get(hash);
...
ChainDB.prototype.hasTX = async function hasTX(hash) {
  ...
  return await this.b.tx.has(hash);
};
...
ChainDB.prototype.getCoinsByAddress = async function getCoinsByAddress(addrs) {
  ...
  for (const addr of addrs) {
    ...
    const keys = await this.b.icoin.keys({
      gte: [hash, encoding.ZERO_HASH, 0],
      lte: [hash, encoding.MAX_HASH, 0xffffffff]
    });
...
ChainDB.prototype.getHashesByAddress = async function getHashesByAddress(addrs) {
  ...
  for (const addr of addrs) {
    ...
    const keys = await this.b.itx.keys({
      gte: [hash, encoding.ZERO_HASH],
      lte: [hash, encoding.MAX_HASH],
      parse: ([, hash]) => {
        hashes[hash] = true;
      }
    });
...
ChainDB.prototype._save = async function _save(entry, block, view) {
  ...
  // Hash->height index.
  this.b.height.put(hash, U32(entry.height));

  // Entry data.
  this.b.entry.put(hash, entry.toRaw());
  this.cacheHash.push(entry.hash, entry);

  // Tip index.
  this.b.tips.del(entry.prevBlock);
  this.b.tips.put(hash, null);
  ...
  if (!entry.isGenesis())
    this.b.next.put(entry.prevBlock, hash);

  // Height->hash index.
  this.b.hash.put(entry.height, hash);
  ...
  this.b.state.put(this.pending.commit(hash));
};
...
ChainDB.prototype._disconnect = async function _disconnect(entry, block) {
  ...
  this.b.next.del(entry.prevBlock);

  // Remove height->hash index.
  this.b.hash.del(entry.height);
  ...
  this.b.state.put(this.pending.commit(entry.prevBlock));
...
ChainDB.prototype.saveUpdates = function saveUpdates() {
  ...
  for (const update of updates) {
    ...
    this.b.bit.put(bit, hash, update.toRaw());
  }
};
...
ChainDB.prototype.reset = async function reset(block) {
  ...
  for (;;) {
    ...
    if (tip.hash === entry.hash) {
      this.b.state.put(this.pending.commit(tip.hash));
      await this.commit();
      break;
    }
    ...
    this.b.tips.del(tip.hash);
    this.b.tips.put(tip.prevBlock, null);

    // Remove all records (including
    // main-chain-only records).
    this.b.hash.del(tip.height);
    this.b.height.del(tip.hash);
    this.b.entry.del(tip.hash);
    this.b.next.del(tip.prevBlock);
    ...
    // Revert chain state to previous tip.
    this.b.state.put(this.pending.commit(tip.prevBlock));
...
ChainDB.prototype.saveBlock = async function saveBlock(entry, block, view) {
  ...
  this.b.block.put(hash, block.toRaw());
...
ChainDB.prototype.saveView = function saveView(view) {
  for (const [hash, coins] of view.map) {
    for (const [index, coin] of coins.outputs) {
      ...
      this.b.coin.put(hash, index, raw);
...
ChainDB.prototype.connectBlock = async function connectBlock(entry, block, view) {
  ...
  if (!view.undo.isEmpty())
    this.b.undo.put(hash, view.undo.commit());
...
ChainDB.prototype.indexTX = function indexTX(tx, view, entry, index) {
  ...
  if (this.options.indexTX) {
    ...
    this.b.tx.put(hash, meta.toRaw());

    if (this.options.indexAddress) {
      const hashes = tx.getHashes(view);
      for (const addr of hashes)
        this.b.itx.put(addr, hash, null);
    }
  }
  ...
  if (!tx.isCoinbase()) {
    for (const {prevout} of tx.inputs) {
      ...
      this.b.icoin.del(addr, prevout.hash, prevout.index);
    }
  }
  ...
  for (let i = 0; i < tx.outputs.length; i++) {
    ...
    this.b.icoin.put(addr, hash, i, null);
  }
};
...
ChainDB.prototype.unindexTX = function unindexTX(tx, view) {
  ...
  if (!tx.isCoinbase()) {
    for (const {prevout} of tx.inputs) {
      ...
      this.b.icoin.put(addr, prevout.hash, prevout.index, null);
    }
  }
