...
Chain.prototype.getDeployments = co(function* getDeployments(ts, prev) {
  ...
  // UASF (BIP148) is now active.
  if (this.options.uasf) {
    // UASF uses the _current_ header's MTP instead of the
    // previous block's (not sure why, this seems to make
    // mining and mempool checks harder). So, we have to
    // hack in some new behavior for getMedianTime().
    mtp = yield prev.getMedianTime(ts);

    if (mtp >= 1501545600 // Tue 01 Aug 2017 00:00:00 UTC
        && mtp <= 1510704000 // Wed 15 Nov 2017 00:00:00 UTC
        && !(yield this.isLockedIn(prev, deployments.segwit))
        && !(yield this.isActive(prev, deployments.segwit))) {
      state.uasf = true;
    }
  }
...
/**
 * Check whether a versionbits deployment is locked in (BIP9: versionbits).
 * @example
 * yield chain.isLockedIn(tip, deployments.segwit);
 * @method
 * @see https://github.com/bitcoin/bips/blob/master/bip-0009.mediawiki
 * @param {ChainEntry} prev - Previous chain entry.
 * @param {Object} deployment - Deployment params.
 * @returns {Promise}
 */

Chain.prototype.isLockedIn = co(function* isLockedIn(prev, deployment) {
  var state = yield this.getState(prev, deployment);
  return state === common.thresholdStates.LOCKED_IN;
});
...
/**
 * Test whether bip148 (UASF) is active.
 * @returns {Boolean}
 */

DeploymentState.prototype.hasUASF = function hasUASF() {
  return this.uasf;
};
