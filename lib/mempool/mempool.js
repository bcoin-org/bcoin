...
Mempool.prototype.getCoinsByAddress = function getCoinsByAddress(addrs) {
  var out = [];
  var addr, hash, coins, coin;

  if (!Array.isArray(addrs))
    addrs = [addrs];

  for (addr of addrs) {
    hash = Address.getHash(addr, 'hex');
    coins = this.coinIndex.get(hash);
...
Mempool.prototype.handleOrphans = async function handleOrphans(parent) {
  ...
  var orphan, tx, missing;

  for (orphan of resolved) {
    try {
      tx = orphan.toTX();
    } catch (e) {
