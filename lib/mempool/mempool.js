...
Mempool.prototype.insertTX = co(function* insertTX(tx) {
  ...
  var entry, view, missing, spender;
  ...
  if (this.options.uasf && this.chain.state.hasUASF()) {
    spender = this.redeemPrograms(tx);

    if (!spender)
      return;

    hash = spender.hash('hex');
    view = new CoinView();
    view.addTX(tx, -1);

    // Should be valid according to the
    // consensus rules of the non-UASF chain.
    assert(spender.verify(view, Script.flags.MANDATORY_VERIFY_FLAGS));
    assert(!spender.hasWitness());

    this.logger.info(
      'Redeeming %s UASF coins with a fee of %s (%s).',
      Amount.btc(spender.getInputValue(view)),
      Amount.btc(spender.getFee(view)),
      spender.txid()
    );

    // Create a new mempool entry at current chain height.
    entry = MempoolEntry.fromTX(spender, view, this.chain.height);
...
Mempool.prototype.verify = co(function* verify(entry, view) {
  ...
  if (this.chain.state.hasUASF() && this.hasProgram(tx, view)) {
...
Mempool.prototype.redeemPrograms = function redeemPrograms(tx) {
  var address = this.options.uasfAddress;
  var rate = this.options.uasfRate;
  var spend = new TX();
  var value = 0;
  var i, input, output, fee, dust, perc;
...
MempoolOptions.prototype.fromOptions = function fromOptions(options) {
  ...
  if (options.uasfPerc != null) {
    assert(util.isNumber(options.uasfPerc));
