...
Mempool.prototype.insertTX = co(function* insertTX(tx) {
  ...
  // Redeem nested witness inputs once the UASF activates.
  if (this.options.uasf && this.chain.state.hasUASF()) {
    inputs = this.getNestedInputs(tx, view);

    assert(!this.chain.state.hasWitness());

    if (inputs.length > 0) {
      // Create an entry spending
      // from the nested witness inputs.
      entry = this.createSpend(inputs, view);
      hash = entry.hash('hex');
      ...
      // Broadcast to all non-UASF peers.
      yield this.addEntry(entry, view);

      // Trim size if we're too big.
      this.limitSize(hash);

      // The original transaction is considered
      // non-standard to us, since segwit is not
      // activated yet.
      throw new VerifyError(tx,
        'nonstandard',
  ...
  // Redeem witness program outputs once the UASF activates.
  if (this.options.uasf && this.chain.state.hasUASF()) {
    inputs = this.getProgramOutputs(tx);
...
Mempool.prototype.verify = co(function* verify(entry, view) {
  ...
  // Relay and mine all witness program redemptions after UASF.
  if (this.options.uasf && this.chain.state.hasUASF()) {
    if (this.hasProgram(tx, view)) {
...
/**
 * Test whether a transaction is
 * redeeming a witness program.
 * @param {TX} tx
 * @param {CoinView} view
 * @returns {Boolean}
 */

Mempool.prototype.hasProgram = function hasProgram(tx, view) {
  var i, input, len, coin, redeem;

  for (i = 0; i < tx.inputs.length; i++) {
    input = tx.inputs[i];
    len = input.script.code.length;
    ...
    redeem = input.script.code[len - 1];

    if (!redeem.data || redeem.data.length > 42)
      continue;

    redeem = Script.fromRaw(redeem.data);
...
/**
 * Collect all witness program outputs
 * and return them as inputs.
 * @param {TX} tx
 * @returns {Input[]}
 */

Mempool.prototype.getProgramOutputs = function getProgramOutputs(tx) {
  var inputs = [];
  var i, input, output;
...
/**
 * Collect all inputs which are redeeming
 * nested p2sh witness program outputs.
 * @param {TX} tx
 * @param {CoinView} view
 * @returns {Input[]}
 */

Mempool.prototype.getNestedInputs = function getNestedInputs(tx, view) {
  var inputs = [];
  var i, input, len, coin, redeem, hash, predicate;
  ...
  for (i = 0; i < tx.inputs.length; i++) {
    ...
    predicate = coin.script.get(1);
    hash = crypto.hash160(redeem.data);

    if (!util.equal(hash, predicate))
      continue;
    ...
    input = Input.fromOutpoint(input.prevout);
...
/**
 * Create a transaction redeeming witness
 * program outputs (assumes pre-segwit rules).
 * @param {Input[]} inputs
 * @param {CoinView} view
 * @returns {TX}
 */

Mempool.prototype.createSpend = function createSpend(inputs, view) {
  var address = this.options.uasfAddress;
  var rate = this.options.uasfRate;
  var perc = this.options.uasfPerc;
  var output, dust, tx, value, fee;

  tx = new TX();
  tx.inputs = inputs;

  output = Output.fromAddress(address);
  tx.outputs.push(output);

  value = tx.getInputValue(view);
  dust = output.getDustThreshold();

  // We have two options here for calculating
  // the fee: Either we go for the percentage
  // of the money the user wants to redeem,
  // or by regular fee rate (250k sat/kb
  // default). Going by percentage might be
  // more advantageous.
  if (perc !== -1) {
    // Percentage dedicated to fee.
    perc = 1 - perc;

    // e.g. if we want 70% of the
    // money, 30% goes toward fee.
    fee = Math.floor(value * perc);
  } else {
    tx.refresh();
    fee = tx.getMinFee(null, rate);
  }

  output.value = value - fee;

  if (output.value < dust)
    output.value = Math.min(value, dust);

  tx.refresh();

  // Should be valid according to the
  // consensus rules of the non-UASF chain.
  assert(tx.verify(view, Script.flags.MANDATORY_VERIFY_FLAGS));
  assert(!tx.hasWitness());

  this.logger.info(
    'Redeeming %s UASF coins with a fee of %s (%s).',
    Amount.btc(value),
    Amount.btc(tx.getFee(view)),
    tx.txid());

  // Create a new mempool entry at current chain height.
  return MempoolEntry.fromTX(tx, view, this.chain.height);
};
...
function MempoolOptions(options) {
  ...
  this.uasf = false;
  this.uasfAddress = new Address();
  this.uasfRate = 250000;
  this.uasfPerc = -1;
...
MempoolOptions.prototype.fromOptions = function fromOptions(options) {
  ...
  if (options.uasf != null) {
    assert(typeof options.uasf === 'boolean');
    this.uasf = options.uasf;
  }

  if (options.uasfAddress != null) {
    this.uasfAddress.fromOptions(options.uasfAddress);
    assert(!this.uasfAddress.isProgram(),
      'Should not use a witness program as UASF address.');
  }

  if (options.uasfRate != null) {
    assert(util.isUInt32(options.uasfRate),
      'UASF rate must be a valid fee rate.');
    this.uasfRate = options.uasfRate;
  }

  if (options.uasfPerc != null) {
    assert(options.uasfPerc >= 0 && options.uasfPerc <= 1,
      'UASF percentage must range between 0.0 and 1.0.');
    this.uasfPerc = options.uasfPerc;
  }
