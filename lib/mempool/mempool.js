...
Mempool.prototype.insertTX = async function insertTX(tx, id) {
  ...
  const [sane, reason, score] = tx.checkSanity();
  ...
  if (this.options.requireStandard) {
    const [standard, reason, score] = tx.checkStandard();
  ...
  // Get conflicts.
  const [valid, conflicts] = this.getConflicts(tx);
  ...
  const ancestors = await this.verify(entry, view);

  // Handle conflicts.
  if (conflicts.size > 0) {
    // Transaction cannot depend on
    // a transaction it would replace.
    for (const hash of conflicts) {
    ...
    // Try to evict all conflicting transactions.
    if (!this.evictConflicts(entry, conflicts)) {
      throw new VerifyError(tx,
        'insufficientfee',
        'insufficient fee',
        0);
  ...
  if (conflicts.size > 0)
    this.emit('replaced', tx, conflicts);
...
Mempool.prototype.verify = async function verify(entry, view) {
  ...
  const ancestors = this.collectAncestors(entry);
  if (ancestors.size + 1 > this.options.maxAncestors) {
    throw new VerifyError(tx,
      'nonstandard',
 ...
 * Get conflicts.
 * @param {TX} tx
 * @returns {Array} [valid, conflicts]
 */

Mempool.prototype.getConflicts = function getConflicts(tx) {
  ...
  for (const {prevout} of tx.inputs) {
    ...
    if (spent && !conflicts.has(hash)) {
      if (!this.options.replaceByFee || !spent.isRBF())
...
Mempool.prototype.evictConflicts = function evictConflicts(entry, conflicts) {
  let fee = 0;
  let size = 0;

  for (const hash of conflicts) {
    const conflict = this.getEntry(hash);
    assert(conflict);
    fee += conflict.deltaFee;
    size += conflict.descSize;
  }

  // Entry must have a fee rate better
  // than everything it would replace.
  if (entry.getDescRate() <= policy.getRate(size, fee))
    return false;

  this.logger.warning(
    'Replacing %d transactions for %s.',
    conflicts.size,
    entry.txid());
...
/**
 * Collect all ancestor hashes for an entry.
 * @param {MempoolEntry} entry
 * @returns {Set}
 */

Mempool.prototype.collectAncestors = function collectAncestors(entry) {
  return this._collectAncestors(entry, new Set(), entry, nop);
};
...
Mempool.prototype.countAncestors = function countAncestors(entry) {
  return this._collectAncestors(entry, new Set(), entry, nop).size;
};
...
Mempool.prototype.updateAncestors = function updateAncestors(entry, map) {
  return this._collectAncestors(entry, new Set(), entry, map).size;
};

/**
 * Traverse ancestors and collect.
 * @private
 * @param {MempoolEntry} entry
 * @param {Object} set
...
Mempool.prototype._collectAncestors = function _collectAncestors(entry, set, child, map) {
  const tx = entry.tx;
  ...
  for (const {prevout} of tx.inputs) {
    ...
    this._collectAncestors(parent, set, child, map);
