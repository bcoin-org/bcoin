...
FullNode.prototype._init = function _init() {
  ...
  this.chain.hook('connect', async function(entry, block) {
    try {
      await self.mempool._addBlock(entry, block.txs);
    } catch (e) {
      self.error(e);
    }
  ...
  this.chain.hook('disconnect', async function(entry, block) {
    try {
      await self.mempool._removeBlock(entry, block.txs);
    } catch (e) {
      self.error(e);
    }
  ...
  this.chain.hook('reset', async function(tip) {
    try {
      await self.mempool._reset();
    } catch (e) {
      self.error(e);
    }
    self.emit('reset', tip);
...
FullNode.prototype.getCoinsByAddress = async function getCoinsByAddress(addresses) {
  var mempool = this.mempool.getCoinsByAddress(addresses);
  var chain = await this.chain.db.getCoinsByAddress(addresses);
  var out = [];
  var i, coin, spent;
...
FullNode.prototype.getMetaByAddress = async function getTXByAddress(addresses) {
  var mempool = this.mempool.getMetaByAddress(addresses);
  var chain = await this.chain.db.getMetaByAddress(addresses);
...
FullNode.prototype.getTXByAddress = async function getTXByAddress(addresses) {
...
FullNode.prototype.getTX = async function getTX(hash) {
  var mtx = await this.getMeta(hash);
  if (!mtx)
    return;
  return mtx.tx;
};
