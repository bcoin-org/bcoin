...
FullNode.prototype.getCoinsByAddress = async function getCoinsByAddress(addrs) {
  var mempool = this.mempool.getCoinsByAddress(addrs);
  var chain = await this.chain.db.getCoinsByAddress(addrs);
  var out = [];
  var coin, spent;
...
FullNode.prototype.getMetaByAddress = async function getTXByAddress(addrs) {
  var mempool = this.mempool.getMetaByAddress(addrs);
  var chain = await this.chain.db.getMetaByAddress(addrs);
...
FullNode.prototype.getTXByAddress = async function getTXByAddress(addrs) {
  var mtxs = await this.getMetaByAddress(addrs);
  var out = [];
  var mtx;
