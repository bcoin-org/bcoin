...
class FullNode extends Node {
  ...
  async getBlockFilterHeader(hash) {
    if (!this.filterindex)
      return null;
    ...
    return this.filterindex.getFilterHeader(hash);
  }

  /**
   * Retrieve compact filter headers by filterType, startHeight and stopHeight.
   * @param {Number} filterType
   * @param {Number} startHeight
   * @param {Hash} stopHash
   * @returns {Promise} - Returns {@link Object(
   *   filterType: Number,
   *   stopHash: Hash,
   *   previousFilterHeader: Buffer,
   *   filterHeaderHashes: hash[]
   * )}.
   */

  async getBlockFilterHeaders(filterType, startHeight, stopHash) {
    if (!stopHash)
      return null;
    ...
    if (typeof startHeight === 'number')
      if (startHeight > stopHeight || stopHeight - startHeight >= 2000)
        return null;

    const headerHashes = [];

    const firstBlockHash = await this.chain.getHash(startHeight);
    const firstBlockHeader = await this.getBlockFilterHeader(firstBlockHash);

    if (firstBlockHeader == null)
      return null;

    const prevBlockHash = firstBlockHeader.prevBlock;
    const prevBlockHeader = await this.getBlockFilterHeader(prevBlockHash);

    for (let i = startHeight; i <= stopHeight; i++) {
      const hash = await this.chain.getHash(i);
    ...
    return {
      ...
      stopHash: stopHash,
      previousFilterHeader: prevBlockHeader,
      filterHashesLength: headerHashes.length,
      filterHashes: headerHashes
    };
  }
}
