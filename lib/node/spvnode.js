...
SPVNode.prototype._init = function _init() {
  ...
  this.chain.on('connect', async function(entry, block) {
    if (self.rescanJob) {
      try {
        await self.watchBlock(entry, block);
      } catch (e) {
        self.error(e);
      }
      return;
...
SPVNode.prototype.scan = async function scan(start, filter, iter) {
  var unlock = await this.scanLock.lock();
...
SPVNode.prototype.watchBlock = async function watchBlock(entry, block) {
  var unlock = await this.watchLock.lock();
  try {
