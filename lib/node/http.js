...
class HTTP extends Server {
  ...
  initRouter() {
    ...
    this.get('/filter/:type/:block', async (req, res) => {
      const valid = Validator.fromRequest(req);
      const hash = valid.uintrhash('block');
      const type = valid.u32('type');

      enforce(hash != null, 'Hash or height required.');
      enforce(type != null, 'Filter type required.');
      enforce(this.index != null, 'Compact filter indexing disabled');
      enforce(!this.index.options.indexFilters,
        'Compact filter indexing disabled');
      ...
      const cfilter = await this.index.getCFilter(block.hash(), type);
      ...
      res.json(200, {'filter': cfilter.toString('hex')});
    });

    // Filter headers by hash/height
    this.get('/cfheader/:type/:block', async (req, res) => {
      const valid = Validator.fromRequest(req);
      const hash = valid.uintrhash('block');
      const type = valid.u32('type');
      ...
      const cfheader = await this.index.getCFHeader(block.hash(), type);
      ...
      res.json(200, {'cfheader': cfheader.toString('hex')});
    });
