...
class HTTP extends Server {
  ...
  initRouter() {
    ...
    // CfHeaders by startHeight, FilterType, and stopHash

    this.get('/cfheaders/:type/:startHeight/:stopHash', async (req, res) => {
      const valid = Validator.fromRequest(req);
      const filterType = valid.u8('filterType');
      const startHeight = valid.u32('startHeight');
      const stopHash = valid.brhash('stopHash');

      enforce(filterType != null, 'FilterType is required.');
      // TODO check if filterType is equal to the basic filter type
      enforce(startHeight != null, 'startHeight is required.');
      enforce(stopHash != null, 'stopHash is required.');
      enforce(!this.chain.options.spv, 'Cannot get filters in SPV mode.');

      const cfheaders = await this
        .node.getBlockFilterHeaders(filterType, startHeight, stopHash);
      ...
      res.json(200, {'cfheaders': cfheaders.toString('hex')});
    });
