...
class HTTP extends Server {
  ...
  initRouter() {
    ...
    this.get('/filter/:type/:block', async (req, res) => {
      ...
      enforce(this.node.bip157 != null, 'Compact filter indexing disabled');
      ...
      const cfilter = await this.node.getCFilter(block.hash(), type);
    ...
    this.get('/cfheader/:type/:block', async (req, res) => {
      ...
      const cfheader = await this.node.getCFHeader(block.hash(), type);
