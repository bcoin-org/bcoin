...
const constants = require('../net/common');
...
class RPC extends RPCBase {
  ...
  init() {
    ...
    this.add('getcfilter', this.getCFilter);
    this.add('getcfilterheader', this.getCFilterHeader);
  ...
  async getCFilter(args, help) {
    if (help || args.length < 1 || args.length > 2)
      throw new RPCError(errs.MISC_ERROR, 'getcfilter "hash" type');

    if (!this.index || !this.index.options.indexFilters)
      throw new RPCError(errs.MISC_ERROR, 'Compact filter indexing disabled');

    const valid = new Validator(args);
    const hash = valid.rhash(0);
    const type = valid.u32(1);

    if (!hash)
      throw new RPCError(errs.MISC_ERROR, 'Invalid block hash.');

    if (typeof type !== 'number')
      throw new RPCError(errs.MISC_ERROR, 'Invalid filter type.');

    if (Object.values(constants.FILTERS).indexOf(type) === -1)
      throw new RPCError(errs.MISC_ERROR, 'Unknown filter type.');

    const cfilter = await this.filterindex.getCFilter(hash, type);

    if (!cfilter)
      throw new RPCError(errs.MISC_ERROR, 'CFilter not found');

    return cfilter.toString('hex');
  }

  async getCFilterHeader(args, help) {
    if (help || args.length < 1 || args.length > 2)
      throw new RPCError(errs.MISC_ERROR, 'getcfilterheader "hash" type');
    ...
    const cfheader = await this.filterindex.getCFHeader(hash, type);

    if (!cfheader)
      throw new RPCError(errs.MISC_ERROR, 'CFilterHeader not found');

    return cfheader.reverse().toString('hex');
  }
