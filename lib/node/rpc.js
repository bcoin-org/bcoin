...
class RPC extends RPCBase {
  ...
  init() {
    ...
    this.add('getfiltercount', this.getFilterCount);
    this.add('getfilterheadercount', this.getFilterHeaderCount);
    ...
    this.add('getblockfilterheader', this.getBlockFilterHeader);
  ...
  async getFilterCount(args, help) {
    if (help || args.length !== 0)
  ...
  async getBlockFilterHeader(args, help) {
    if (help || args.length < 1 || args.length > 2) {
      throw new RPCError(errs.MISC_ERROR,
        'getblockfilterheader "hash" ( "type" )');
    }
    ...
    const filterHeader = await this.node.getBlockFilterHeader(hash, filterName);

    if (!filterHeader)
      throw new RPCError(errs.MISC_ERROR, 'Block filter header not found.');
