...
RBTIterator.prototype.init = function init() {
  ...
  if (this.options.reverse) {
    if (this.options.lte)
      iter.seekMax(this.options.lte);
    else
      iter.seekLast();
  } else {
    if (this.options.gte)
      iter.seekMin(this.options.gte);
    else
      iter.seekFirst();
  }
...
RBTIterator.prototype.next = function next() {
  ...
  if (options.reverse) {
    ...
    if (result && options.gte) {
      if (options.gt) {
        if (iter.compare(options.gte) <= 0)
          result = false;
      } else {
        if (iter.compare(options.gte) < 0)
          result = false;
    ...
    if (result && options.lte) {
      if (options.lt) {
        if (iter.compare(options.lte) >= 0)
          result = false;
      } else {
        if (iter.compare(options.lte) > 0)
          result = false;
