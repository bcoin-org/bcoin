...
Pool.prototype.handleAddr = co(function* handleAddr(peer, addrs) {
...
Pool.prototype.handleInv = co(function* handleInv(peer, items) {
  var unlock = yield this.locker.lock();
  try {
    return yield this._handleInv(peer, items);
  } finally {
    unlock();
  }
});
...
Pool.prototype._handleInv = co(function* handleInv(peer, items) {
  var blocks = [];
  var txs = [];
  var unknown = -1;
  var i, item;
...
Pool.prototype.handleReject = co(function* handleReject(peer, reject) {
  var entry;
  ...
  entry = this.invMap.get(reject.hash);
...
Pool.prototype.handleNotFound = co(function* handleNotFound(peer, items) {
  var i, item;
...
Pool.prototype.handleGetData = co(function* handleGetData(peer, items) {
  var notFound = [];
  var txs = 0;
  var blocks = 0;
  var unknown = -1;
...
Pool.prototype.handleCmpctBlock = co(function* handleCmpctBlock(peer, block) {
  var hash = block.hash('hex');
  ...
  if (result) {
    this.logger.debug(
      ...
      block.rhash(), peer.hostname);
    yield this.handleBlock(peer, block.toBlock());
    return;
  }
...
Peer.prototype.handleBlockTxn = co(function* handleBlockTxn(peer, res) {
  ...
  if (!block.fillMissing(res)) {
    peer.increaseBan(100);
    this.logger.warning('Peer sent non-full blocktxn (%s).', peer.hostname);
    return;
  }
...
Pool.prototype.handleGetBlockTxn = co(function* handleGetBlockTxn(peer, req) {
