...
class Pool extends EventEmitter {
  ...
  async handlePacket(peer, packet) {
    switch (packet.type) {
      ...
      case packetTypes.NOTFOUND:
        ...
        break;
      case packetTypes.GETCFTYPES:
        await this.handleGetCFTypes(peer, packet);
        break;
      case packetTypes.CFTYPES:
        await this.handleCFTypes(peer, packet);
        break;
      ...
      case packetTypes.CFHEADERS:
        ...
        break;
      case packetTypes.GETCFILTER:
        await this.handleGetCFilter(peer, packet);
        break;
  ...
  /**
   * Handle `getcftypes` packet.
   * @method
   * @private
   * @param {Peer} peer
   * @param {GetCFTypesPacket} packet
   */

  async handleGetCFTypes(peer, packet) {
    if (!(this.bip157 || this.chain.synced))
      return;

    peer.send(new packets.CFTypesPacket([
  ...
  /**
   * Handle `cftypes` packet.
   * @method
   * @private
   * @param {Peer} peer
   * @param {CFTypesPacket} packet
   */

  async handleCFTypes(peer, packet) {
    ;
  }
  ...
  async handleGetCFHeaders(peer, packet) {
    if (!(this.bip157 || this.chain.synced || !this.index))
      return;
    ...
    while (entry) {
      const cfheader = await this.index.getCFHeader(
        entry.hash, packet.filterType);
      cfheaders.push(cfheader);

      if (entry.hash === packet.stop)
        break;

      if (cfheaders.length === 2000)
        break;

      entry = await this.chain.getNext(entry);
    }

    for (let i = 0; i < cfheaders.length; i += 2000) {
      const chunk = cfheaders.slice(i, i + 2000);
  ...
  /**
   * Handle `getcfilter` packet.
   * @method
   * @private
   * @param {Peer} peer
   * @param {GetCFilterPacket} packet
   */

  async handleGetCFilter(peer, packet) {
    if (!(this.bip157 || this.chain.synced || !this.index))
      return;
    ...
    const cfilter = await this.index.getCFilter(
      packet.hash,
      packet.filterType
    );
  ...
  */
  ...
  async getCFHeaders(tip, stop) {
    ...
    for (let peer = this.peers.head(); peer; peer = peer.next) {
      ...
      peer.sendGetCFHeaders(locator, stop, common.FILTERS.EXTENDED);
    }
  }
