...
class Pool extends EventEmitter {
  ...
  async startFilterHeadersSync() {
    ...
    const startHeight = this.chain.db.neutrinoState.headerHeight + 1;
  ...
  async handleCFHeaders(peer, packet) {
    ...
    this.logger.info('Received cfheaders (%s).', peer.hostname());
    ...
    console.log(this.headerChain.tail.height, stopHeight);
    if (this.headerChain.tail.height <= stopHeight) {
      this.logger.info('CFHeaders sync complete.');
      this.emit('cfheaders');
    } else {
  ...
  async handleCFilter(peer, packet) {
    ...
    this.logger.info('Received cfilter (%s).', peer.hostname());
    const filterType = packet.filterType;
