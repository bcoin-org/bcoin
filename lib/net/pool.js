...
Pool.prototype.getNextTip = function getNextTip(height) {
  ...
  for (next of this.network.checkpoints) {
...
Pool.prototype.handleBlockInv = async function handleBlockInv(peer, hashes) {
  ...
  if (exists && this.chain.synced) {
    height = await this.chain.db.getHeight(exists);
    if (height !== -1)
      peer.bestHeight = height;
  }
...
Pool.prototype.handleGetData = async function handleGetData(peer, packet) {
  ...
  var compact = 0;
  var unknown = -1;
  var item, tx, block, result, height;
  ...
  for (item of items) {
    if (item.isTX()) {
      tx = await this.getItem(peer, item);
    ...
    switch (item.type) {
      ...
      case invTypes.WITNESS_BLOCK:
        result = await this.sendBlock(peer, item, item.hasWitness());
        if (!result) {
          notFound.push(item);
          continue;
        }
      ...
      case invTypes.CMPCT_BLOCK:
        height = await this.chain.db.getHeight(item.hash);

        // Fallback to full block.
        if (height < this.chain.tip.height - 10) {
          result = await this.sendBlock(peer, item, peer.compactWitness);
          if (!result) {
            notFound.push(item);
            continue;
          }
...
Pool.prototype._handleHeaders = async function handleHeaders(peer, packet) {
  ...
  var header, hash, height, last, node;
...
Pool.prototype.resolveOrphan = async function resolveOrphan(peer, orphan) {
  var locator = await this.chain.getLocator();
...
Pool.prototype.getHeaders = async function getHeaders(peer, tip, stop) {
  var locator = await this.chain.getLocator(tip);
