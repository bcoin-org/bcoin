...
class Pool extends EventEmitter {
  ...
  constructor(options) {
    ...
    this.filterindex = this.options.filterindex;;
  ...
  async handlePacket(peer, packet) {
    switch (packet.type) {
      ...
      case packetTypes.CFILTER:
        await this.handleCFilter(peer, packet);
        break;
  ...
  /**
   * Handle `cfilter` packet.
   * @method
   * @private
   * @param {Peer} peer
   * @param {CFilterPacket} packet
   */

  async handleCFilter(peer, packet) {
    ;
  }

  /**
   * Handle `cfheaders` packet.
   * @method
   * @private
   * @param {Peer} peer
   * @param {CFHeadersPacket} packet
   */

  async CFHeadersPacket(peer, packet) {
    ;
  }

  /**
   * Handle `cfcheckpt` packet.
   * @method
   * @private
   * @param {Peer} peer
  ...
  async CFCheckpt(peer, packet) {
    ;
  }

  /**
   * Handle `getcfheaders` packet.
   * @method
   * @private
   * @param {Peer} peer
  ...
  async handleGetCFHeaders(peer, packet) {
    if (!(this.bip157 || this.chain.synced || !this.filterindex))
      return;
    ...
    if (packet.filterType !== common.FILTERS.REGULAR)
      return;

    const stop = await this.chain.getEntryByHash(
      Buffer.from(packet.stopHash, 'hex'));
    const hashes = await this.chain.getHashes(
      packet.startHeight-1, stop.height);

    if (hashes.length === 0 ||
        packet.startHeight > 0 && hashes.length === 1)
      return;

    const filterHashes = await this.filterindex.getCFilterHashesByBlockHashes(
      packet.filterType,
      hashes
    );

    let prevHeader = null;
    if (packet.startHeight > 0) {
      prevHeader = await this.filterindex.getCFHeader(
      hashes[0], packet.filterType);

      hashes.shift();
      filterHashes.shift();
    }

    peer.send(new packets.CFHeadersPacket(
      packet.filterType,
      packet.stopHash,
      prevHeader,
      filterHashes
  ...
  async handleGetCFilters(peer, packet) {
    ...
    const cfilters = await this.filterindex.getCFiltersByBlockHashes(
      packet.filterType,
      hashes
    );

    for (let i = 0; i < cfilters.length; i++) {
      peer.send(new packets.CFilterPacket(
        packet.filterType,
        hashes[i],
        cfilters[i]
      ));
  ...
  async handleGetCFCheckpt(peer, packet) {
    ...
    const stop = await this.chain.getEntry(Buffer.from(packet.stopHash, 'hex'));

    const hashes = [];

    for (let i = Math.floor(stop.height / 1000); i > 0; i--) {
      const height = i * 1000;
    ...
    const filterHeaders = await this.filterindex.getCFHeadersByBlockHashes(
      packet.filterType,
      hashes
    );

    peer.send(new packets.CFCheckptPacket(
      packet.filterType,
      packet.stopHash,
      filterHeaders
    ));
  ...
  /**
  * Send `getcfheaders` to peer after building locator.
  * @method
  * @param {Peer} peer
  ...
  */
  ...
  async getCFHeaders(tip, stop) {
    const locator = await this.chain.getLocator(tip);
    for (let peer = this.peers.head(); peer; peer = peer.next) {
      peer.sendGetCFHeaders(locator, stop, common.FILTERS.REGULAR);
    }
  }
...
class PoolOptions {
  ...
  fromOptions(options) {
    ...
    if (options.filterindex != null) {
      assert(typeof options.filterindex === 'object');
    ...
    if (this.bip157)
      this.services |= common.services.CF;
