...
Pool.prototype.handleBlocks = co(function* handleBlocks(hashes, peer) {
  var i, hash;
...
Pool.prototype.handleInv = co(function* handleInv(hashes, peer) {
  var unlock = yield this.locker.lock();
  try {
    return yield this._handleInv(hashes, peer);
  } finally {
    unlock();
  }
});
...
Pool.prototype._handleInv = co(function* handleInv(hashes, peer) {
  var i, hash;
  ...
  if (!this.options.headers) {
    yield this.handleBlocks(hashes, peer);
    return;
  }
...
Pool.prototype.bindPeer = function bindPeer(peer) {
  ...
  peer.on('blocks', co(function* (hashes) {
    ...
    try {
      yield self.handleInv(hashes, peer);
    } catch (e) {
      self.emit('error', e);
    }
  }));
