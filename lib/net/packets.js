...
class VersionPacket extends Packet {
  ...
  write(bw) {
    ...
    this.remote.write(bw, false);
    this.local.write(bw, false);
  ...
  read(br) {
    ...
    this.remote.read(br, false);

    if (br.left() > 0) {
      this.local.read(br, false);
...
class AddrPacket extends Packet {
  ...
  read(br) {
    ...
    for (let i = 0; i < count; i++)
      this.items.push(NetAddress.read(br, true));
...
exports.decode = function decode(cmd, data) {
  switch (cmd) {
    case 'version':
    ...
    default:
      return UnknownPacket.decode(data, cmd);
  }
};
