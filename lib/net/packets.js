...
class GetCFiltersPacket extends Packet {
  ...
  toWriter(bw) {
    ...
    bw.writeHash(this.stopHash || encoding.ZERO_HASH);
    return bw;
  }
...
class CFilterPacket extends Packet {
  ...
  constructor(filterType, hash, data) {
    super();
  ...
  toWriter(bw) {
    assert(this.data.length <= 256 * 1024, 'Too large data.');
 ...
 * @property {Number} filter type
 * @property {Hash} stop hash
 * @property {Hash} prev filter header
 * @property {Hash[]} filter hashes
 */
...
class CFHeadersPacket extends Packet {
   ...
   * @param {Number} filter type
   * @param {Hash?} stop hash
   * @param {Hash?} prev filter header
   * @param {(Hash[])?} filter hashes
   */

  constructor(filterType, stopHash, prevHeader, filterHashes) {
    super();
    ...
    this.filterType = filterType;
    this.stopHash = stopHash || null;
    this.prevHeader = prevHeader || null;
  ...
  getSize() {
    ...
    size += 32 * this.filterHashes.length;
    return size;
  }
  ...
  toWriter(bw) {
    assert(this.filterHashes.length <= 2000, 'Too many cfheaders.');

    bw.writeU8(this.filterType);
    bw.writeHash(this.stopHash || encoding.ZERO_HASH);
    bw.writeHash(this.prevHeader || encoding.ZERO_HASH);
  ...
  fromReader(br) {
    this.filterType = br.readU8();
    this.stopHash = br.readHash('hex');
    this.prevHeader = br.readHash('hex');

    const count = br.readVarint();

    assert(count <= 2000, 'Too many cfheaders.');

    for (let i = 0; i < count; i++)
      this.filterHashes.push(br.readHash('hex'));
...
class CFCheckptPacket extends Packet {
  ...
  getSize() {
    ...
    size += 32 * this.filterHeaders.length;
    return size;
  }
  ...
  toWriter(bw) {
    assert(this.filterHeaders.length <= 100000, 'Too many filter headers.');
  ...
  fromReader(br) {
    ...
    assert(count <= 100000, 'Too many filter headers.');

    for (let i = 0; i < count; i++)
      this.filterHeaders.push(br.readHash('hex'));
