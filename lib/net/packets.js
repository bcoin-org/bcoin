...
class GetCFTypesPacket extends Packet {
  /**
   * Create a `getcftypes` packet.
   * @constructor
   */
  ...
  constructor() {
    super();
    this.cmd = 'getcftypes';
    this.type = exports.types.GETCFTYPES;
  }

  /**
   * Instantiate getcftypes packet from serialized data.
   * @param {Buffer} data
   * @param {String?} enc
   * @returns {GetCFTypesPacket}
   */
 ...
 * CFTypes Packet
 * @extends Packet
 * @property {Number[]} filter types
 */

class CFTypesPacket extends Packet {
  /**
   * Create a `cftypes` packet.
   * @constructor
   * @param {(Number[])?} filter types
   */
  ...
  constructor(filterTypes) {
    ...
    this.cmd = 'cftypes';
    this.type = exports.types.CFTYPES;

    this.cftypes = filterTypes || [];
  }
  ...
  getSize() {
    let size = 0;
    size += encoding.sizeVarint(this.cftypes.length);
    size += this.cftypes.length;
    return size;
  }

  /**
   * Serialize cftypes packet to writer.
   * @param {BufferWriter} bw
   */

  toWriter(bw) {
    assert(this.cftypes.length <= 256, 'Too many cftypes.');

    bw.writeVarint(this.cftypes.length);

    for (const item of this.cftypes)
      bw.writeU8(item);
  ...
  fromReader(br) {
    ...
    assert(count <= 256, 'Too many cftypes.');

    for (let i = 0; i < count; i++)
      this.cftypes.push(br.readU8());
  ...
  /**
   * Instantiate cftypes packet from buffer reader.
   * @param {BufferReader} br
   * @returns {CFTypesPacket}
   */
  ...
  /**
   * Instantiate cftypes packet from serialized data.
   * @param {Buffer} data
   * @param {String?} enc
   * @returns {CFTypesPacket}
   */
 ...
 * @property {Number} filter type
 */

class GetCFilterPacket extends Packet {
  /**
   * Create a `getcfilter` packet.
   * @constructor
   * @param {Hash} block hash
   * @param {Number} filter type
   */

  constructor(hash, filterType) {
    super();

    this.cmd = 'getcfilter';
    this.type = exports.types.GETCFILTER;
  ...
  /**
   * Serialize getcfilter packet to writer.
   * @param {BufferWriter} bw
   */

  toWriter(bw) {
    bw.writeHash(this.hash || encoding.ZERO_HASH);
    bw.writeU8(this.filterType);
    return bw;
  }

  /**
   * Serialize getcfilter packet.
   * @returns {Buffer}
   */
  ...
  /**
   * Instantiate getcfilter packet from buffer reader.
   * @param {BufferReader} br
   * @returns {GetCFilterPacket}
   */
  ...
  /**
   * Instantiate getcfilter packet from serialized data.
   * @param {Buffer} data
   * @param {String?} enc
   * @returns {GetCFilterPacket}
   */
...
class CFilterPacket extends Packet {
  ...
  constructor(hash, filterType, data) {
    super();
...
class GetCFHeadersPacket extends Packet {
   ...
   * @constructor
   * @param {(Hash[])?} locator
   * @param {Hash?} stop hash
   * @param {Number} filter type
   */

  constructor(locator, stop, filterType) {
    super();
...
class CFHeadersPacket extends Packet {
   ...
   * @param {Hash?} stop hash
   * @param {Number} filter type
   * @param {(Hash[])?} header hashes
   */

  constructor(stop, filterType, hashes) {
    super();
  ...
  toWriter(bw) {
    assert(this.hashes.length <= 50000, 'Too many cfheaders.');
  ...
  fromReader(br) {
    ...
    assert(count <= 50000, 'Too many cfheaders.');

    for (let i = 0; i < count; i++)
      this.hashes.push(br.readHash('hex'));
...
exports.fromRaw = function fromRaw(cmd, data) {
  switch (cmd) {
    ...
    case 'getcftypes':
      return GetCFTypesPacket.fromRaw(data);
    case 'cftypes':
      return CFTypesPacket.fromRaw(data);
    case 'getcfilter':
      return GetCFilterPacket.fromRaw(data);
    case 'cfilter':
...
exports.GetCFTypesPacket = GetCFTypesPacket;
exports.CFTypesPacket = CFTypesPacket;
exports.GetCFilterPacket = GetCFilterPacket;
