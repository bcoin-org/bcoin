...
class Peer extends EventEmitter {
  ...
  /**
   * Send `getcftypes` to peer.
   */

  sendGetCFTypes() {
    this.send(new packets.GetCFTypesPacket());
  }

  /**
   * Send `getcfheaders` to peer. Note that it can have a null locator.
   * @param {Hash[]?} locator - Chain locator.
   * @param {Hash?} stop - Hash to stop at.
   * @param {Number} type - Filter type.
   */

  sendGetCFHeaders(locator, stop, type) {
    const packet = new packets.GetCFHeadersPacket(locator, stop, type);

    let hash = null;
    if (packet.locator.length > 0)
      hash = format.revHex(packet.locator[0]);

    let end = null;
    if (stop)
      end = format.revHex(stop);

    this.logger.debug(
      'Requesting cfheaders packet from peer with getcfheaders (%s).',
      this.hostname());

    this.logger.debug(
      'Sending getcfheaders (hash=%s, stop=%s).',
      hash, end);
  ...
  /**
   * Send `getcfilter` to peer.
   * @param {Hash} hash - Block hash.
   * @param {Number} type - Filter type.
   */

  sendGetCFilter(hash, type) {
    const packet = new packets.GetCFilterPacket(hash, type);

    this.logger.debug(
      'Sending getcfilter (hash=%s, type=%s).',
      hash, type);
