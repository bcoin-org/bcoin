...
AESKey.prototype.encryptBlock = function encryptBlock(input) {
  let output, kp, key, r, s0, s1, s2, s3, t0, t1, t2, t3;
...
AESCipher.prototype.final = function final() {
  ...
  if (!this.waiting) {
    ...
    let left = 16 - this.waiting.length;
    let pad = Buffer.allocUnsafe(left);
    pad.fill(left);
...
AES.encrypt = function encrypt(data, key, iv, bits, mode) {
  let cipher = new AESCipher(key, iv, bits, mode);
...
AES.decrypt = function decrypt(data, key, iv, bits, mode) {
  let decipher = new AESDecipher(key, iv, bits, mode);
...
function xor(v1, v2) {
  let out = Buffer.allocUnsafe(v1.length);
