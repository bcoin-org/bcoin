...
Wallet.prototype.getPrivateKey = function getPrivateKey(enc) {
  ...
  if (enc === 'base58') {
    ...
    var arr = [bcoin.protocol.constants.prefixes.privateKey];
...
Wallet.hash2addr = function hash2addr(hash) {
  ...
  // Add version
  hash = [bcoin.protocol.constants.prefixes.publicKey].concat(hash);
...
Wallet.addr2hash = function addr2hash(addr) {
  ...
  if (addr.length !== 25)
    return [];
  if (addr[0] !== bcoin.protocol.constants.prefixes.publicKey)
    return [];
...
Wallet.fromJSON = function fromJSON(json) {
  ...
  if (json.priv) {
    ...
    assert.equal(key[0], bcoin.protocol.constants.prefixes.privateKey);
