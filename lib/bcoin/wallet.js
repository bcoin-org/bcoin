...
function Wallet(options) {
  ...
  if (!this.accountKey) {
    ...
    key = this.master.key.deriveAccount44(this.accountIndex);
    this.accountKey = key.hdPublicKey;
  }
...
Wallet.prototype.pushKey = function pushKey(key) {
  var result = false;
  var index = -1;
  var i;
...
/**
 * Remove a public account key to the wallet (multisig).
 * Does not update the database.
 * @param {HDPublicKey} key - Account (bip44)
 * key (can be in base58 form).
 * @throws Error on non-hdkey/non-accountkey.
...
Wallet.prototype.spliceKey = function spliceKey(key) {
  var result = false;
  var index = -1;
  var i;
...
Wallet.prototype.sign = function sign(tx, options, callback) {
  ...
  this.deriveInputs(tx, function(err, addresses) {
    ...
    master = master.deriveAccount44(self.accountIndex);
