...
Wallet.prototype.init = function init(callback) {
  ...
  function done(err, account) {
    ...
    if (self.options.passphrase)
      self.master.encrypt(self.options.passphrase);

    if (Buffer.isBuffer(self.options.passphrase))
      self.options.passphrase.fill(0);

    self.options.passphrase = null;
  ...
  this.getOutputPaths(tx, function(err, paths) {
    ...
    utils.forEachSerial(Object.keys(accounts), function(index, next) {
      ...
      self.getAccount(+index, function(err, account) {
        ...
        account.setChangeDepth(depth.changeDepth + 1, function(err, res) {
          if (err)
            return next(err);
          ...
          account.setReceiveDepth(depth.receiveDepth + 1, function(err, res) {
            if (err)
              return next(err);
...
Account.prototype.setReceiveDepth = function setReceiveDepth(depth, callback) {
  var self = this;
  var addresses = [];
  var i;
...
MasterKey.fromRaw = function fromRaw(raw) {
  ...
  if (!data.encrypted)
    data.key = bcoin.hd.PrivateKey.fromRaw(data.xprivkey);
