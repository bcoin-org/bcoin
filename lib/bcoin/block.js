...
Block.prototype._checkBlock = function checkBlock() {
  ...
  var sigops = 0;
  // TODO:
  // for (var i = 0; this.txs.length; i++) {
  //   for (var j = 0; j < this.txs.inputs.length; j++) {
  //     var script = this.txs.inputs[j].script;
  //     sigops += countSigops(script);
  //   }
  //   for (var j = 0; j < this.txs.outputs.length; j++) {
  //     var script = this.txs.outputs[j].script;
  //     sigops += countSigops(script);
  //   }
  // }
  if (sigops > bcoin.protocol.constants.block.maxSigops) {
    return false;
  }
