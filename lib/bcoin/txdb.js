...
function PathInfo(tx, table) {
  ...
  // All output wallet IDs (for balance & syncing).
  this.wallets = [];
...
Details.prototype._insert = function _insert(vector, target, id, table) {
...
Details.prototype.init = function init(id, table) {
  this._insert(this.tx.inputs, this.inputs, id, table);
  this._insert(this.tx.outputs, this.outputs, id, table);
};
