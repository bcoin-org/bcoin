...
Witness.prototype.isPubkeyhashInput = function isPubkeyhashInput() {
  return this.items.length === 2
    && Script.isSignature(this.items[0])
    && Script.isKey(this.items[1]);
};
...
Script.prototype.refresh = function refresh() {
...
Script.prototype.execute = function execute(stack, flags, tx, index, version) {
  ...
  for (ip = 0; ip < this.code.length; ip++) {
    ...
    if (op.data) {
      ...
      if (negate === 0) {
        if (!Script.checkMinimal(op.data, op.value, flags))
...
Script.prototype.removeData = function removeData(data) {
  ...
  for (i = 0; i < this.code.length; i++) {
    ...
    if (!Script.checkMinimal(op.data, op.value))
      continue;
...
Script.checkMinimal = function checkMinimal(data, opcode, flags) {
  if (flags == null)
    flags = constants.flags.STANDARD_VERIFY_FLAGS;
...
Script.prototype.isWitnessPubkeyhash = function isWitnessPubkeyhash() {
  ...
  return this.raw[0] === opcodes.OP_0 && this.raw.length === 22;
};
...
Script.prototype.isWitnessScripthash = function isWitnessScripthash() {
  ...
  return this.raw[0] === opcodes.OP_0 && this.raw.length === 34;
};
...
Script.getCoinbaseHeight = function getCoinbaseHeight(raw) {
  ...
  return new bn(raw.slice(1, 1 + raw[0]), 'le').toNumber();
};
...
Script.prototype.getCoinbaseFlags = function getCoinbaseFlags() {
  ...
  if (this.code[1].data && this.code[1].data.length <= 6) {
    coinbase.extraNonce = new bn(this.code[1].data, 'le').toNumber();
  } else {
    nonce = this.getSmall(1);
    coinbase.extraNonce = nonce === -1 ? -1 : nonce;
  }
  ...
  return code.map(function(op) {
    ...
    if (data) {
      ...
      if (!constants.opcodesByVal[value]) {
        ...
        if (value.length < 2)
          value = '0' + value;
        return '0x' + value + ' 0x' + data.toString('hex');
      }

      value = constants.opcodesByVal[value];
      return value + ' 0x' + size + ' 0x' + data.toString('hex');
    }

    assert(typeof value === 'number');

    if (constants.opcodesByVal[value])
      return constants.opcodesByVal[value];
  ...
  for (i = 0; i < code.length; i++) {
    ...
    if (opcodes[symbol] == null) {
      if (op[0] === '\'') {
        ...
        p.writeBytes(Script.encodeArray([op]));
        continue;
      }
      if (/^-?\d+$/.test(op)) {
        op = new bn(op, 10);
...
ScriptWriter.prototype.writePush = function writePush(data, op) {
  var p = this.writer;
...
ScriptWriter.prototype.writeNumber = function writeNumber(num) {
  ...
  if (num >= 1 && num <= 16) {
    this.writeOp(num + 0x50);
    return;
  }

  if (num === -1) {
    this.writeOp(opcodes.OP_1NEGATE);
    return;
  }
