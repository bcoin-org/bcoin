...
RPC.prototype.getinfo = function getinfo(args) {
  return spawn(function *() {
    ...
    return {
      ...
      testnet: this.network.type !== bcoin.network.main,
      keypoololdest: 0,
      keypoolsize: 0,
...
RPC.prototype.getmininginfo = function getmininginfo(args) {
  return spawn(function *() {
    ...
    return {
      ...
      testnet: this.network !== bcoin.network.main,
      chain: 'main',
...
RPC.prototype.getrawtransaction = function getrawtransaction(args) {
  ...
  if (args.help || args.length < 1 || args.length > 2)
    return Promise.reject(new RPCError('getrawtransaction "txid" ( verbose )'));
  ...
  var tx = yield this.node.getTX(hash);

  if (!tx)
    return Promise.reject(new RPCError('Transaction not found.'));
...
RPC.prototype.signmessagewithprivkey = function signmessagewithprivkey(args) {
  ...
  key = bcoin.keyring.fromSecret(key);
  msg = new Buffer(RPC.magic + msg, 'utf8');
  msg = crypto.hash256(msg);
