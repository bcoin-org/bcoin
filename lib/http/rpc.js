...
RPC.prototype.getChainTips = async function getChainTips(args, help) {
  var hash, tips, result, entry, fork, main;
...
RPC.prototype.getTXOutProof = async function getTXOutProof(args, help) {
  ...
  if (hash) {
    ...
    tx = await this.chain.db.getMeta(txid);
    if (!tx)
      return;
    block = await this.chain.db.getBlock(tx.block);
  } else {
    coins = await this.chain.db.getCoins(txid);
    if (!coins)
      return;
...
RPC.prototype.verifyTXOutProof = async function verifyTXOutProof(args, help) {
  ...
  for (hash of block.tree.matches)
...
RPC.prototype._createWork = async function _createWork() {
  var attempt = await this.updateWork();
  var n1 = this.nonce1;
  var n2 = this.nonce2;
  var ts = attempt.ts;
  var data, root, head;
...
RPC.prototype._createTemplate = async function _createTemplate(maxVersion, coinbase, rules) {
  var attempt = await this.getTemplate();
  ...
  for (deploy of this.network.deploys) {
    state = await this.chain.getState(this.chain.tip, deploy);
    name = deploy.name;
...
RPC.prototype.createRawTransaction = async function createRawTransaction(args, help) {
  ...
  var tx, input, output, hash, index, sequence;
...
RPC.prototype.addBlock = async function addBlock(block) {
  var unlock1 = await this.locker.lock();
  var unlock2 = await this.chain.locker.lock();
  try {
...
RPC.prototype._addBlock = async function _addBlock(block) {
  ...
  if (prev) {
    state = await this.chain.getDeployments(block.ts, prev);
...
RPC.prototype.getBIP9Softforks = async function getBIP9Softforks() {
  ...
  var forks = {};
  var deployment, state, status;

  for (deployment of this.network.deploys) {
    state = await this.chain.getState(tip, deployment);
...
RPC.prototype.mineBlocks = async function mineBlocks(blocks, address, tries) {
  var unlock = await this.locker.lock();
  try {
    return await this._mineBlocks(blocks, address, tries);
  } finally {
    unlock();
  }
};

RPC.prototype._mineBlocks = async function _mineBlocks(blocks, address, tries) {
  var hashes = [];
  var i, block;

  for (i = 0; i < blocks; i++) {
    block = await this.miner.mineBlock(null, address);
...
RPC.prototype.headerToJSON = async function headerToJSON(entry) {
  var medianTime = await entry.getMedianTime();
  var nextHash = await this.chain.db.getNextHash(entry.hash);
...
RPC.prototype.blockToJSON = async function blockToJSON(entry, block, details) {
  var mtp = await entry.getMedianTime();
  var nextHash = await this.chain.db.getNextHash(entry.hash);
  var txs = [];
  var tx, json;
