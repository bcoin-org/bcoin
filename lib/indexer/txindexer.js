...
class TXIndexer extends Indexer {
  ...
  async indexBlock(meta, block, view) {
    ...
    this.put(layout.b.encode(meta.height), brecord.encode());
    ...
    for (let i = 0; i < block.txs.length; i++) {
      ...
      this.put(layout.t.encode(hash), txrecord.encode());
    }
  }
  ...
  async getMeta(hash) {
    ...
    const record = TxRecord.decode(raw);
    ...
    const brecord = BlockRecord.decode(braw);
