...
class Wallet extends EventEmitter {
  ...
  deriveReceive(acct = 0, index) {
    return this.deriveKey(acct, 0, index);
  }
  ...
  deriveChange(acct = 0, index) {
    return this.deriveKey(acct, 1, index);
  }
  ...
  deriveNested(acct = 0, index) {
    return this.deriveKey(acct, 2, index);
  }
  ...
  async deriveKey(acct, branch, index) {
    const unlock = await this.writeLock.lock();
    try {
      return await this._deriveKey(acct, branch, index);
    } finally {
      unlock();
    }
  }
  ...
  /**
   * Derive an address (doesn't increments depth) without a lock.
   * @private
   * @param {(Number|String)?} acct
   * @param {Number} branch
   * @param {Number} index
  ...
  async _deriveKey(acct, branch, index) {
    const account = await this.getAccount(acct);
    ...
    const b = this.db.batch();
    const key = await account.createKeyAtIndex(b, branch, index);
    await b.write();
