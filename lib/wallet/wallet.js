...
class Wallet extends EventEmitter {
   ...
   * @param {Address} options.changeAddress - Use the specified change
   * address rather than generating a new one.
   */
  ...
  async _fund(mtx, options) {
    ...
    let change = options.changeAddress;
    if (change == null) {
      change = await this.changeAddress(acct);
