...
class Wallet extends EventEmitter {
   ...
   * @param {Object?} options
   * @param {(String|Number)?} options.account - If no account is
   * specified, coins from the default account will be used to fill.
   * Pass -1 to fill from the entire wallet.
   * @param {String?} options.selection - Coin selection priority. Can
   * be `age`, `random`, or `all`. (default=age).
   * @param {Boolean} options.round - Whether to round to the nearest
   * kilobyte for fee calculation.
  ...
  async _fill(mtx, coins, options) {
    ...
    if (options.account) {
      assert(
        typeof options.account === 'number',
        'Account option should be a number corresponding to index of account'
      );
    ...
    let address;
    if (addressCount) {
      const index = Math.floor(Math.random() * (addressCount));
      address = options.addresses[index];
      assert(
        address,
        'Problem getting address for sweep from address list'
      );
    } else {
      // if no addresses passed and sweeping entire wallet
      // will sweep to address of default account
      const index = accountIndex < 0 ? 0 : accountIndex;
      address = await this.receiveAddress(index);
    }
  ...
  * @param {(String|Number)?} options.account - If no account is
  * specified, defaults to default account, index = 0. Pass in
  * `account = -1` to sweep entire wallet
  ...
  */
  ...
  async _sweep(options, passphrase) {
    let account = 0;
    // note you can pass in account = -1 to sweep whole wallet
    if (options.account) {
      assert(
        typeof options.account === 'number',
        'Account option should be a number corresponding to index of account'
