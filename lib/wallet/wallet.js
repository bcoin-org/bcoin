...
class Wallet extends EventEmitter {
   ...
   * @param {Boolean} options.replaceable - Signal BIP 125 replaceability
  ...
  async createTX(options, force) {
    ...
    // Opt in BIP 125 replaceability
    if (options.replaceable !== false)
  ...
  async bumpTXFee(hash, rate, sign, passphrase) {
    ...
    // Require explicit opt-in RBF signalling (for now) BIP 125 rule #1
    if (!tx.isRBF())
      throw new Error('Transaction does not signal opt-in replace-by-fee.');

    // No child spends
    // Covers BIP 125 rule #5 and makes rule #3 fee easier to compute
    for (let index = 0; index < tx.outputs.length; index++) {
      if (await this.txdb.isSpent(wtx.hash, index))
        throw new Error('Transaction has descendants in the wallet.');
    }
    ...
    // Find the original TX change output
    let change = null;
    for (let i = 0; i < mtx.outputs.length; i++) {
      ...
      if (path.branch === 1) {
        // Edge case if (for example) a customer requests a withdrawl
        // to one of our own change addresses.
        // TODO: Allow bumpTXFee() caller to specify an output to reduce.
        if (change)
          throw new Error('Found more than one change address.');
        change = output;
        mtx.changeIndex = i;
      }
    }
    ...
    this.logger.debug(
      'Bumping fee for wallet tx (%s): replacing %h with %h',
      this.id,
      wtx.hash,
      ntx.hash());
