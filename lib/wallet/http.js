...
HTTPServer.prototype.initRouter = function initRouter() {
  ...
  this.hook(async function(req, res) {
    var valid = req.valid();
    var id, token, wallet;
  ...
  // Rescan
  this.post('/_admin/rescan', async function(req, res) {
    var valid = req.valid();
  ...
  // Resend
  this.post('/_admin/resend', async function(req, res) {
    await this.walletdb.resend();
    res.send(200, { success: true });
  });

  // Backup WalletDB
  this.post('/_admin/backup', async function(req, res) {
    var valid = req.valid();
  ...
  // List wallets
  this.get('/_admin/wallets', async function(req, res) {
    var wallets = await this.walletdb.getWallets();
  ...
  // Create wallet
  this.put('/:id', async function(req, res) {
    var valid = req.valid();
    var wallet;
  ...
  // List accounts
  this.get('/:id/account', async function(req, res) {
    var accounts = await req.wallet.getAccounts();
    res.send(200, accounts);
  });

  // Get account
  this.get('/:id/account/:account', async function(req, res) {
    var valid = req.valid();
    var acct = valid.str('account');
    var account = await req.wallet.getAccount(acct);
  ...
  this.post('/:id/account', async function(req, res) {
    var valid = req.valid();
  ...
  // Create account
  this.put('/:id/account/:account', async function(req, res) {
    var valid = req.valid();
    var passphrase = valid.str('passphrase');
    var options, account;
  ...
  // Change passphrase
  this.post('/:id/passphrase', async function(req, res) {
    var valid = req.valid();
  ...
  // Unlock wallet
  this.post('/:id/unlock', async function(req, res) {
    var valid = req.valid();
    var passphrase = valid.str('passphrase');
  ...
  // Lock wallet
  this.post('/:id/lock', async function(req, res) {
    await req.wallet.lock();
    res.send(200, { success: true });
  });

  // Import key
  this.post('/:id/import', async function(req, res) {
    var valid = req.valid();
    var acct = valid.str('account');
  ...
  // Generate new token
  this.post('/:id/retoken', async function(req, res) {
    var valid = req.valid();
    var passphrase = valid.str('passphrase');
    var token = await req.wallet.retoken(passphrase);
    res.send(200, { token: token.toString('hex') });
  });

  // Send TX
  this.post('/:id/send', async function(req, res) {
    var valid = req.valid();
    var passphrase = valid.str('passphrase');
    var outputs = valid.array('outputs');
    var options, tx, details, output, script;
  ...
  // Create TX
  this.post('/:id/create', async function(req, res) {
    var valid = req.valid();
    var passphrase = valid.str('passphrase');
    var outputs = valid.array('outputs');
    var options, tx, output, script;
  ...
  // Sign TX
  this.post('/:id/sign', async function(req, res) {
    var valid = req.valid();
    var passphrase = valid.str('passphrase');
  ...
  // Zap Wallet TXs
  this.post('/:id/zap', async function(req, res) {
    var valid = req.valid();
    var acct = valid.str('account');
  ...
  // Abandon Wallet TX
  this.del('/:id/tx/:hash', async function(req, res) {
    var valid = req.valid();
  ...
  // List blocks
  this.get('/:id/block', async function(req, res) {
    var heights = await req.wallet.getBlocks();
    res.send(200, heights);
  });

  // Get Block Record
  this.get('/:id/block/:height', async function(req, res) {
    var valid = req.valid();
    var height = valid.u32('height');
    var block;
  ...
  // Add key
  this.put('/:id/shared-key', async function(req, res) {
    var valid = req.valid();
    var acct = valid.str('account');
  ...
  // Remove key
  this.del('/:id/shared-key', async function(req, res) {
    var valid = req.valid();
    var acct = valid.str('account');
    var key = valid.str('accountKey');
    enforce(key, 'Key is required.');
  ...
  // Get key by address
  this.get('/:id/key/:address', async function(req, res) {
    var valid = req.valid();
    var address = valid.str('address');
    var key;
  ...
  // Get private key
  this.get('/:id/wif/:address', async function(req, res) {
    var valid = req.valid();
    var address = valid.str('address');
    var passphrase = valid.str('passphrase');
    var key;
  ...
  // Create address
  this.post('/:id/address', async function(req, res) {
    var valid = req.valid();
    var acct = valid.str('account');
    var address = await req.wallet.createReceive(acct);
    res.send(200, address.toJSON());
  });

  // Create change address
  this.post('/:id/change', async function(req, res) {
    var valid = req.valid();
    var acct = valid.str('account');
    var address = await req.wallet.createChange(acct);
    res.send(200, address.toJSON());
  });

  // Create nested address
  this.post('/:id/nested', async function(req, res) {
    var valid = req.valid();
    var acct = valid.str('account');
    var address = await req.wallet.createNested(acct);
    res.send(200, address.toJSON());
  });

  // Wallet Balance
  this.get('/:id/balance', async function(req, res) {
    var valid = req.valid();
    var acct = valid.str('account');
    var balance = await req.wallet.getBalance(acct);
  ...
  // Wallet UTXOs
  this.get('/:id/coin', async function(req, res) {
    var valid = req.valid();
    var acct = valid.str('account');
    var coins = await req.wallet.getCoins(acct);
    var result = [];
    var coin;
  ...
  // Locked coins
  this.get('/:id/locked', async function(req, res) {
  ...
  // Lock coin
  this.put('/:id/locked/:hash/:index', async function(req, res) {
    var valid = req.valid();
    var hash = valid.hash('hash');
  ...
  // Unlock coin
  this.del('/:id/locked/:hash/:index', async function(req, res) {
    var valid = req.valid();
    var hash = valid.hash('hash');
    var index = valid.u32('index');
    var outpoint;
  ...
  // Wallet Coin
  this.get('/:id/coin/:hash/:index', async function(req, res) {
    var valid = req.valid();
    var hash = valid.hash('hash');
    var index = valid.u32('index');
    var coin;
  ...
  // Wallet TXs
  this.get('/:id/tx/history', async function(req, res) {
    var valid = req.valid();
    var acct = valid.str('account');
    var txs = await req.wallet.getHistory(acct);
    var result = [];
    var details, item;
  ...
  // Wallet Pending TXs
  this.get('/:id/tx/unconfirmed', async function(req, res) {
    var valid = req.valid();
    var acct = valid.str('account');
    var txs = await req.wallet.getPending(acct);
    var result = [];
    var details, item;
  ...
  this.get('/:id/tx/range', async function(req, res) {
    var valid = req.valid();
    var acct = valid.str('account');
    var result = [];
    var options, txs, details, item;
  ...
  // Last Wallet TXs
  this.get('/:id/tx/last', async function(req, res) {
    var valid = req.valid();
    var acct = valid.str('account');
    var limit = valid.u32('limit');
    var txs = await req.wallet.getLast(acct, limit);
    var details = await req.wallet.toDetails(txs);
    var result = [];
    var item;
  ...
  // Wallet TX
  this.get('/:id/tx/:hash', async function(req, res) {
    var valid = req.valid();
    var hash = valid.hash('hash');
    var tx, details;
  ...
  // Resend
  this.post('/:id/resend', async function(req, res) {
...
HTTPServer.prototype.handleAuth = function handleAuth(socket) {
  ...
  socket.hook('wallet join', async function(args) {
    var valid = new Validator([args]);
    var id = valid.str(0, '');
    ...
    try {
      wallet = await self.walletdb.auth(id, token);
    } catch (e) {
