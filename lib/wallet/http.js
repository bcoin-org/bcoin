...
class HTTP extends Server {
  ...
  initRouter() {
   ...
   // Derive receive address
    this.post('/wallet/:id/derivereceive', async (req, res) => {
      const valid = Validator.fromRequest(req);
      const acct = valid.str('account');
      const index = valid.u32('index');
      const addr = await req.wallet.deriveReceive(acct, index);

      res.json(200, addr.toJSON(this.network));
    });

    // Derive change address
    this.post('/wallet/:id/derivechange', async (req, res) => {
      const valid = Validator.fromRequest(req);
      const acct = valid.str('account');
      const index = valid.u32('index');
      const addr = await req.wallet.deriveChange(acct, index);

      res.json(200, addr.toJSON(this.network));
    });

    // Derive nested address
    this.post('/wallet/:id/derivenested', async (req, res) => {
      const valid = Validator.fromRequest(req);
      const acct = valid.str('account');
      const index = valid.u32('index');
      const addr = await req.wallet.deriveNested(acct, index);
