...
class Account {
  ...
  toJSON(balance) {
    ...
    const nestedKey = this.nestedKey();

    const receiveAddressDetails = {};
    const changeAddressDetails = {};
    const nestedAddressDetails = {};

    if (receiveKey) {
      receiveAddressDetails.redeemScript = receiveKey.script ?
        receiveKey.script.toRaw().toString('hex') : null;
      receiveAddressDetails.publicKey = receiveKey.publicKey ?
        receiveKey.publicKey.toString('hex') : null;
      receiveAddressDetails.program = receiveKey.getProgram() ?
      receiveKey.getProgram().toRaw().toString('hex') : null;
    }

    if (changeKey) {
      changeAddressDetails.redeemScript = changeKey.script ?
      changeKey.script.toRaw().toString('hex') : null;
      changeAddressDetails.publicKey = changeKey.publicKey ?
      changeKey.publicKey.toString('hex') : null;
      changeAddressDetails.program = changeKey.getProgram() ?
      changeKey.getProgram().toRaw().toString('hex') : null;
    }

    if (nestedKey) {
      nestedAddressDetails.redeemScript = nestedKey.script ?
      nestedKey.script.toRaw().toString('hex') : null;
      nestedAddressDetails.publicKey = nestedKey.publicKey ?
      nestedKey.publicKey.toString('hex') : null;
      nestedAddressDetails.program = nestedKey.getProgram() ?
      nestedKey.getProgram().toRaw().toString('hex') : null;
    }
    ...
    return {
      ...
      nestedAddress: nested ? nested.toString(this.network) : null,
      nestedAddressDetails: nestedAddressDetails,
      accountKey: this.accountKey.toBase58(this.network),
      keys: this.keys.map(key => key.toBase58(this.network)),
