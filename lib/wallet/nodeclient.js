...
class NodeClient extends AsyncEmitter {
  ...
  async setFilter(filter, height) {
    ...
    // With an SPV node, if the filter was updated from a
    // confirmed block, the pool will need to re-download blocks
    // from peers with the updated bloom filter. A wallet running
    // as a plugin does not use a bloom filter between the wallet
    // and the node, and does not need to receive the blocks again
    // with the filter updated.
    if (this.node.spv) {
      if (height && this.node.chain.height > height)
        await this.node.chain.detach(height);
    }
  }
