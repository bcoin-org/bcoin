...
function TXDB(wallet) {
  ...
  p[0] = 't'.charCodeAt(0);
  p.writeUInt32LE(wallet.id, 1, true);
  ...
  this.b = {
    R: this.db.bucket('R', [], p),
    t: this.db.bucket('t', ['hash256'], p),
    c: this.db.bucket('c', ['hash256', 'uint32'], p),
    d: this.db.bucket('d', ['hash256', 'uint32'], p),
    s: this.db.bucket('s', ['hash256', 'uint32'], p),
    p: this.db.bucket('p', ['hash256'], p),
    m: this.db.bucket('m', ['uint32', 'hash256'], p),
    h: this.db.bucket('h', ['uint32', 'hash256'], p),

    T: this.db.bucket('T', ['uint32', 'hash256'], p),
    P: this.db.bucket('P', ['uint32', 'hash256'], p),
    M: this.db.bucket('M', ['uint32', 'uint32', 'hash256'], p),
    H: this.db.bucket('H', ['uint32', 'uint32', 'hash256'], p),
    C: this.db.bucket('C', ['uint32', 'hash256', 'uint32'], p),

    r: this.db.bucket('r', ['hash256'], p),
    b: this.db.bucket('b', ['uint32'], p)
  };
...
TXDB.prototype.saveCredit = async function saveCredit(credit, path) {
  ...
  this.b.c.put(coin.hash, coin.index, raw);
  this.b.C.put(path.account, coin.hash, coin.index, null);
...
TXDB.prototype.removeCredit = async function removeCredit(credit, path) {
  ...
  this.b.c.del(coin.hash, coin.index);
  this.b.C.del(path.account, coin.hash, coin.index);
...
TXDB.prototype.spendCredit = function spendCredit(credit, tx, index) {
  ...
  this.b.s.put(prevout.hash, prevout.index, spender.toRaw());
  this.b.d.put(spender.hash, spender.index, credit.coin.toRaw());
};
...
TXDB.prototype.unspendCredit = function unspendCredit(tx, index) {
  const prevout = tx.inputs[index].prevout;
  const spender = Outpoint.fromTX(tx, index);
  this.b.s.del(prevout.hash, prevout.index);
  this.b.d.del(spender.hash, spender.index);
};
...
TXDB.prototype.isRBF = async function isRBF(tx) {
  ...
  for (const {prevout} of tx.inputs) {
    if (await this.b.r.has(prevout.hash))
      return true;
  }
...
TXDB.prototype.getSpent = async function getSpent(hash, index) {
  const data = await this.b.s.get(hash, index);
...
TXDB.prototype.isSpent = function isSpent(hash, index) {
  return this.b.s.has(hash, index);
};
...
TXDB.prototype.getBlock = async function getBlock(height) {
  const data = await this.b.b.get(height);
...
TXDB.prototype.addBlock = async function addBlock(hash, meta) {
  let data = await this.b.b.get(meta.height);
  let block;
  ...
  this.b.b.put(meta.height, block);
};
...
TXDB.prototype.addBlockSlow = async function addBlockSlow(hash, meta) {
  ...
  this.b.b.put(meta.height, block.toRaw());
};
...
TXDB.prototype.removeBlockSlow = async function removeBlockSlow(hash, height) {
  ...
  this.b.b.put(height, block.toRaw());
};
...
TXDB.prototype.insert = async function insert(wtx, block) {
  ...
  this.b.t.put(hash, wtx.toRaw());
  this.b.m.put(wtx.mtime, hash, null);

  if (!block)
    this.b.p.put(hash, null);
  else
    this.b.h.put(height, hash, null);

  // Do some secondary indexing for account-based
  // queries. This saves us a lot of time for
  // queries later.
  for (const account of accounts) {
    this.b.T.put(account, hash, null);
    this.b.M.put(account, wtx.mtime, hash, null);

    if (!block)
      this.b.P.put(account, hash, null);
    else
      this.b.H.put(account, height, hash, null);
  }
  ...
  // to disk.
  this.pending.tx++;
  this.b.R.put(this.pending.commit());
...
TXDB.prototype.erase = async function erase(wtx, block) {
  ...
  this.b.t.del(hash);
  this.b.m.del(wtx.mtime, hash);

  if (!block)
    this.b.p.del(hash);
  else
    this.b.h.del(height, hash);

  // Remove all secondary indexing.
  for (const account of accounts) {
    this.b.T.del(account, hash);
    this.b.M.del(account, wtx.mtime, hash);

    if (!block)
      this.b.P.del(account, hash);
    else
      this.b.H.del(account, height, hash);
  }
...
TXDB.prototype.getAccountHistoryHashes = function getAccountHistoryHashes(account) {
  return this.b.T.keys({
    gte: [account, encoding.NULL_HASH],
    lte: [account, encoding.HIGH_HASH],
    parse: k => k[1]
  });
};
...
TXDB.prototype.getAccountOutpoints = function getAccountOutpoints(account) {
  return this.b.C.keys({
    gte: [account, encoding.NULL_HASH, 0],
    lte: [account, encoding.HIGH_HASH, 0xffffffff],
    parse: ([, hash, index]) => {
...
TXDB.prototype.getAccountHeightRangeHashes = function getAccountHeightRangeHashes(account, options) {
  ...
  return this.b.H.keys({
    gte: [account, start, encoding.NULL_HASH],
    lte: [account, end, encoding.HIGH_HASH],
    limit: options.limit,
...
TXDB.prototype.getHeightRangeHashes = function getHeightRangeHashes(account, options) {
  ...
  return this.b.h.keys({
    gte: [start, encoding.NULL_HASH],
    lte: [end, encoding.HIGH_HASH],
    limit: options.limit,
    reverse: options.reverse,
    parse: k => k[1]
  });
...
TXDB.prototype.getHistory = function getHistory(account) {
  ...
  // Fast case
  return this.b.t.values({
    parse: d => TXRecord.fromRaw(d)
  });
};
...
TXDB.prototype.getTX = async function getTX(hash) {
  const raw = await this.b.t.get(hash);
...
TXDB.prototype.getCredit = async function getCredit(hash, index) {
  ...
  const data = await this.b.c.get(hash, index);
...
TXDB.prototype.getSpentCoin = async function getSpentCoin(spent, prevout) {
  const data = await this.b.d.get(spent.hash, spent.index);
...
TXDB.prototype.hasSpentCoin = function hasSpentCoin(spent) {
  return this.b.d.has(spent.hash, spent.index);
};
...
TXDB.prototype.updateSpentCoin = async function updateSpentCoin(tx, index, height) {
  ...
  this.b.d.put(spent.hash, spent.index, coin.toRaw());
};
...
TXDB.prototype.hasCoin = async function hasCoin(hash, index) {
  ...
  return await this.b.c.has(hash, index);
};
...
TXDB.prototype.abandon = async function abandon(hash) {
  const result = await this.b.p.has(hash);
