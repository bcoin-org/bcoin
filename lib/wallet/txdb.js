...
TXDB.prototype.addBlockSlow = async function addBlockSlow(b, hash, meta) {
  ...
  b.put(layout.b(meta.height), block.toRaw());
};
...
TXDB.prototype.removeBlockSlow = async function removeBlockSlow(b, hash, height) {
...
TXDB.prototype.erase = async function erase(wtx, block) {
  ...
  if (block) {
    ...
    await this.removeBlockSlow(b, hash, height);
  }
