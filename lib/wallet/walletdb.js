...
function WalletDB(options) {
  ...
  this.b = {
    path: this.db.bucket('p', ['hash']),
    wpath: this.db.bucket('P', ['uint32', 'hash']),
    apath: this.db.bucket('r', ['uint32', 'uint32', 'hash']),
    wallet: this.db.bucket('w', ['uint32']),
    wid: this.db.bucket('l', ['string']),
    acct: this.db.bucket('a', ['uint32', 'uint32']),
    aindex: this.db.bucket('i', ['uint32', 'string']),
    aname: this.db.bucket('n', ['uint32', 'uint32']),
    state: this.db.bucket('R'),
    block: this.db.bucket('h', ['uint32']),
    mblock: this.db.bucket('b', ['uint32']),
    outpoint: this.db.bucket('o', ['hash256', 'uint32']),
    txdb: this.db.bucket('t', ['uint32', 'char', 'hash256'])
  };
...
WalletDB.prototype.watch = async function watch() {
  const piter = this.b.path.iterator();

  let hashes = 0;

  while (await piter.next()) {
    try {
      this.filter.add(piter.key, 'hex');
    } catch (e) {
      await piter.end();
      throw e;
    }
  ...
  const oiter = this.b.outpoint.iterator();

  let outpoints = 0;

  while (await oiter.next()) {
    try {
      const [hash, index] = oiter.key;
...
WalletDB.prototype.getDepth = async function getDepth() {
  ...
  // highest wallet wid.
  const iter = this.b.wallet.iterator({
    reverse: true,
    limit: 1
  });
...
WalletDB.prototype.getWalletID = async function getWalletID(id) {
  ...
  const data = await this.b.wid.get(id);
...
WalletDB.prototype._get = async function _get(wid) {
  ...
  const data = await this.b.wallet.get(wid);
...
WalletDB.prototype.save = function save(wallet) {
  ...
  this.b.wallet.put(wid, wallet.toRaw());
  this.b.wid.put(id, U32(wid));
};
...
WalletDB.prototype.renameAccount = function renameAccount(account, name) {
  ...
  this.b.aindex.del(account.wid, account.name);
...
WalletDB.prototype.getAccount = async function getAccount(wid, index) {
  const data = await this.b.acct.get(wid, index);
...
WalletDB.prototype.getAccountIndex = async function getAccountIndex(wid, name) {
  const index = await this.b.aindex.get(wid, name);
...
WalletDB.prototype.getAccountName = async function getAccountName(wid, index) {
  const name = await this.b.aname.get(wid, index);
...
WalletDB.prototype.saveAccount = function saveAccount(account) {
  ...
  // Account data
  this.b.acct.put(wid, index, account.toRaw());

  // Name->Index lookups
  this.b.aindex.put(wid, name, U32(index));

  // Index->Name lookups
  this.b.aname.put(wid, index, Buffer.from(name, 'ascii'));
...
WalletDB.prototype.hasAccount = function hasAccount(wid, index) {
  return this.b.acct.has(wid, index);
};
...
WalletDB.prototype.getPathMap = async function getPathMap(hash) {
  ...
  const data = await this.b.path.get(hash);
...
WalletDB.prototype.savePath = async function savePath(wallet, path) {
  ...
  this.b.path.put(hash, map.toRaw());

  // Wallet ID + Address Hash -> Path Data
  this.b.wpath.put(wid, hash, path.toRaw());

  // Wallet ID + Account Index + Address Hash -> Dummy
  this.b.apath.put(wid, path.account, hash, null);
};
...
WalletDB.prototype.getPath = async function getPath(wid, hash) {
  const data = await this.b.wpath.get(wid, hash);
...
WalletDB.prototype.hasPath = function hasPath(wid, hash) {
  return this.b.wpath.has(wid, hash);
};
...
WalletDB.prototype.getOutpoints = function getOutpoints() {
  return this.b.outpoint.keys({
    parse: ([hash, index]) => new Outpoint(hash, index)
  });
};
...
WalletDB.prototype.getWalletHashes = function getWalletHashes(wid) {
  return this.b.wpath.keys({
    gte: [wid, encoding.NULL_HASH],
    lte: [wid, encoding.HIGH_HASH],
    parse: k => k[1]
  });
};
...
WalletDB.prototype.getAccountHashes = function getAccountHashes(wid, account) {
  return this.b.apath.keys({
    gte: [wid, account, encoding.NULL_HASH],
    lte: [wid, account, encoding.HIGH_HASH],
    parse: k => k[2]
  });
};
...
WalletDB.prototype.getWalletPaths = async function getWalletPaths(wid) {
  const items = await this.b.wpath.range({
    gte: [wid, encoding.NULL_HASH],
    lte: [wid, encoding.HIGH_HASH]
  });
...
WalletDB.prototype.encryptKeys = async function encryptKeys(wallet, key) {
  ...
  for (let path of paths) {
    ...
    this.b.wpath.put(wid, path.hash, path.toRaw());
  }
};
...
WalletDB.prototype.decryptKeys = async function decryptKeys(wallet, key) {
  ...
  for (let path of paths) {
    ...
    this.wpath.put(wid, path.hash, path.toRaw());
  }
};
...
WalletDB.prototype.resend = async function resend() {
  const keys = await this.b.wallet.keys();

  for (const wid of keys)
...
WalletDB.prototype.resendPending = async function resendPending(wid) {
  const keys = await this.b.txdb.keys({
    gte: [wid, 'p', encoding.NULL_HASH],
    lte: [wid, 'p', encoding.HIGH_HASH],
    parse: k => k[2]
  });
  ...
  for (const hash of keys) {
    const data = await this.b.txdb.get(wid, 't', hash);
...
WalletDB.prototype.resetState = async function resetState(tip, marked) {
  ...
  const iter = this.b.block.iterator({
    values: false
  });
  ...
  this.b.block.put(tip.height, tip.toHash());
  this.b.state.put(state.toRaw());
...
WalletDB.prototype.getBlockMap = async function getBlockMap(height) {
  const data = await this.b.mblock.get(height);
...
WalletDB.prototype.writeBlockMap = function writeBlockMap(wallet, height, block) {
  this.b.mblock.put(height, block.toRaw());
};
...
WalletDB.prototype.getOutpointMap = async function getOutpointMap(hash, index) {
  const data = await this.b.outpoint.get(hash, index);
...
WalletDB.prototype.writeOutpointMap = function writeOutpointMap(wallet, hash, index, map) {
  ...
  this.b.outpoint.put(hash, index, map.toRaw());
};
...
WalletDB.prototype.unwriteOutpointMap = function unwriteOutpointMap(wallet, hash, index) {
  this.b.outpoint.del(hash, index);
};
...
WalletDB.prototype.getBlock = async function getBlock(height) {
  const data = await this.b.block.get(height);
...
WalletDB.prototype.revert = async function revert(target) {
  const iter = this.b.mblock.iterator({
    gte: [target + 1],
    reverse: true,
    values: true
  });
