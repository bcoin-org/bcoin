...
class WalletDB extends EventEmitter {
  ...
  /**
   * Send filter to the remote node. If the key was
   * added from a confirmed a block, there could be more
   * transactions in the block and filters need to be
   * updated with the new hash and blocks requested and
   * sent again to find the transactions.
   * @private
   * @returns {Promise}
   */
  ...
  syncFilter() {
    ...
    this.logger.info(
      'Sending filter to server (size=%dmb, height=%d).',
  ...
  async _addBlock(entry, txs) {
    ...
    // Sync the state to the new tip _after_ transactions
    // have been added.
    await this.setTip(tip);
