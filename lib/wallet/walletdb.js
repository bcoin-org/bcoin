...
WalletDB.prototype.load = async function load() {
  var unlock = await this.txLock.lock();
  try {
    await this.connect();
    await this.init();
    await this.watch();
...
WalletDB.prototype.bind = function bind() {
  ...
  this.client.on('block connect', async function(entry, txs) {
    try {
      await self.addBlock(entry, txs);
    } catch (e) {
      self.emit('error', e);
    }
  });

  this.client.on('block disconnect', async function(entry) {
    try {
      await self.removeBlock(entry);
    } catch (e) {
      self.emit('error', e);
    }
  });

  this.client.hook('block rescan', async function(entry, txs) {
    try {
      await self.rescanBlock(entry, txs);
    } catch (e) {
      self.emit('error', e);
    }
  });

  this.client.on('tx', async function(tx) {
    try {
      await self.addTX(tx);
    } catch (e) {
      self.emit('error', e);
  ...
  this.client.on('chain reset', async function(tip) {
    try {
      await self.resetChain(tip);
    } catch (e) {
      self.emit('error', e);
    }
  });
...
WalletDB.prototype.get = async function get(id) {
  var wid = await this.getWalletID(id);
  var unlock;
...
WalletDB.prototype.rename = async function rename(wallet, id) {
  var unlock = await this.writeLock.lock();
  try {
...
WalletDB.prototype._create = async function create(options) {
  var exists = await this.has(options.id);
  var wallet;
...
WalletDB.prototype.ensure = async function ensure(options) {
  var wallet = await this.get(options.id);
  if (wallet)
    return wallet;
...
WalletDB.prototype.getAccount = async function getAccount(wid, index) {
  var data = await this.db.get(layout.a(wid, index));
...
WalletDB.prototype.getAccountIndex = async function getAccountIndex(wid, name) {
  var index = await this.db.get(layout.i(wid, name));
...
WalletDB.prototype.getAccountName = async function getAccountName(wid, index) {
  var name = await this.db.get(layout.n(wid, index));
...
WalletDB.prototype.getPath = async function getPath(wid, hash) {
  var data = await this.db.get(layout.P(wid, hash));
  var path;
...
WalletDB.prototype.getWalletsByTX = async function getWalletsByTX(tx) {
  ...
  var result = [];
  var input, prevout, hash, map, wid;
...
WalletDB.prototype.getBlockMap = async function getBlockMap(height) {
  var data = await this.db.get(layout.b(height));
...
WalletDB.prototype.getOutpointMap = async function getOutpointMap(hash, index) {
  var data = await this.db.get(layout.o(hash, index));
...
WalletDB.prototype.getBlock = async function getBlock(height) {
  var data = await this.db.get(layout.h(height));
  var block;
...
WalletDB.prototype.getTip = async function getTip() {
  var tip = await this.getBlock(this.state.height);
...
WalletDB.prototype._insert = async function insert(tx, block) {
  ...
  wids = await this.getWalletsByTX(tx);
