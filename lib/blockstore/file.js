...
class FileBlockStore extends AbstractBlockStore {
  ...
  /**
   * This method stores block filter data in files.
   * @param {Buffer} hash - The block hash
   * @param {Buffer} header - The block filter header
   * @param {Buffer} filter - The block filter
   * @returns {Promise}
   */

  async writeFilter(hash, header, filter) {
    assert(header.length === 32, 'Invalid header length');

    filter = filter || Buffer.alloc(0);
    const bw = bio.write(header.length + filter.length);
  ...
  /**
   * This method will retrieve block filter data.
   * @param {Buffer} hash - The block hash
   * @returns {Promise}
   */

  async readFilter(hash) {
    return this._read(types.FILTER, hash, 32);
  }

  /**
   * This method will retrieve block filter header.
   * @param {Buffer} hash - The block hash
   * @returns {Promise}
   */
  ...
  /**
   * This will free resources for storing the block filter data.
   * @param {Buffer} hash - The block hash
   * @returns {Promise}
   */
