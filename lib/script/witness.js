...
class Witness extends Stack {
   ...
   * Note that this method does not verify any rules about the control block,
   * that is expected to be evaluated by the caller. This is just a getter.
   * @returns {Buffer|null} BIP341 Control block
   */

  getControlBlock() {
    const spendType = this.getSpendType();

    // Keypath spend: no script, no control block
    if (~spendType & (1 << 1))
      return null;
    ...
    // "The last stack element is called the control block."
