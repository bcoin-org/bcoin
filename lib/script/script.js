...
class Script {
  ...
  /**
   * get required number (`m`) and pubkeys from the multisig script.
   * Similar to the method with the same name in bitcoind.
   */

  matchMultisig() {
    if (this.code.length < 1 || this.getOp(-1) !== opcodes.OP_CHECKMULTISIG)
      return [-1, null];
    const required = this.getInt(0);
    if (required < 0 || required > consensus.MAX_MULTISIG_PUBKEYS)
      return [-1, null];
    const pubkeys = [];
    for (let i = 0; i <= this.length; i++) {
      const pk = this.getData(i);
      if (pk && common.isKeyEncoding(pk)) {
        pubkeys.push(pk);
      }
    }
