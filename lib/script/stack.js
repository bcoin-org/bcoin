...
class Stack {
  ...
  /**
   * Get stack from template.
   * Empty buffers are replaced with witnesses serially.
   * @param {Number|Buffer[]} template
   * @param {Buffer[]} witnesses
   * @returns {Stack}
   */

  static fromTemplate(template, witnesses) {
    let index = 0;
    const stack = new Stack();

    for (const op of template) {
      if (Buffer.from([]).equals(op)) {
        stack.pushData(witnesses[index]);
        index++;
        ...
        throw new Error('template element not Buffer or Integer');
    }
