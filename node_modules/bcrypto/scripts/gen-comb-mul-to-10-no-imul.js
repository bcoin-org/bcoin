...
function genCombMulTo(alen, blen) {
  ...
  for (let k = 0; k < len; k++) {
    ...
    src.push('\/* k = ' + k + ' *\/');
    src.push('lo = al' + (k - minJ) + ' * bl' + minJ + ' | 0;');
    src.push('mid = al' + (k - minJ) + ' * bh' + minJ + ' | 0;');
    src.push(
      'mid = (mid + ah' + (k - minJ) + ' * bl' + minJ + ') | 0;');
    src.push('hi = ah' + (k - minJ) + ' * bh' + minJ + ' | 0;');

    for (let j = minJ + 1; j <= maxJ; j++) {
      const i = k - j;

      src.push('lo = (lo + al' + i + ' * bl' + j + ') | 0;');
      src.push('mid = (mid + al' + i + ' * bh' + j + ') | 0;');
      src.push('mid = (mid + ah' + i + ' * bl' + j + ') | 0;');
      src.push('hi = (hi + ah' + i + ' * bh' + j + ') | 0;');
    }
